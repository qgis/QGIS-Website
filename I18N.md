...
- Coverage refresh CI: `.github/workflows/tx-coverage.yml` runs `scripts/i18n/tx_fetch_coverage.py` (Transifex REST API v3) to update `data/tx_coverage.json` daily.
...
### Protecting Technical Content from Translation

**Problem:** Translators sometimes accidentally translate technical content that should remain in English, such as:
...
- CSS class names: `class="rounded"` → `class="заоблена"`

This breaks the website build and creates maintenance burden.

**Solution:** We use a multi-layered protection strategy:

#### 1. Automated Sanitization (Active Protection)

The `scripts/i18n/sanitize_shortcodes.py` script automatically fixes translated technical content **after** pulling from Transifex but **before** generating content. It runs automatically as part of `make txpull`.

**What it fixes:**
- ✅ Translated shortcode names (e.g., `край на колоната` → `column-end`)
- ✅ Translated file paths in `ref` shortcodes
- ✅ Translated parameter values (`showcase`, `columns`, `class`, `layoutClass`, etc.)
- ✅ Translated technical identifiers

The script:
1. Compares `msgid` (English source) with `msgstr` (translation)
2. Identifies shortcodes and technical parameters
3. Restores English values for technical content
4. Preserves translated display text

Run manually with:
```bash
make sanitize-shortcodes
# or
python3 scripts/i18n/sanitize_shortcodes.py --dry-run  # to preview changes
```

#### 2. Transifex Configuration (Prevention)

Configure Transifex to lock technical patterns so translators cannot modify them. See [TRANSIFEX_CONFIGURATION.md](docs/i18n/TRANSIFEX_CONFIGURATION.md) for detailed setup instructions.

Recommended Transifex settings:
- Enable **Placeholder Detection**
- Add regex patterns to protect shortcodes and file paths
- Provide translator guidelines (see below)

#### 3. Translator Education (Best Practice)

We provide clear guidelines to translators:
- **[TRANSLATORS_GUIDE.md](docs/i18n/TRANSLATORS_GUIDE.md)** - Comprehensive guide with examples
- Instructions in Transifex project settings
- Regular communication with translation teams

**Key rule for translators:** Do NOT translate anything within `{{< >}}` brackets or technical parameter values.

**For Translation Coordinators:** 
- Review [TRANSIFEX_CONFIGURATION.md](docs/i18n/TRANSIFEX_CONFIGURATION.md) to set up Transifex protections
- Share [TRANSLATORS_GUIDE.md](docs/i18n/TRANSLATORS_GUIDE.md) with your translation team
- Monitor sanitize script output to identify recurring issues
...
#### ⚠️ CRITICAL: Shortcode Syntax Requirements for Translation

**See:** [docs/i18n/SHORTCODE_SYNTAX_GUIDE.md](docs/i18n/SHORTCODE_SYNTAX_GUIDE.md) for detailed conversion instructions.
...
1. **Translation Keys in HTML:**
   ...
   - Example: `{{ i18n "exploreQgis" }}` in `themes/qgis-website-theme/layouts/partials/explore.html`
...
3. **Transifex Integration:**
   - Due to different YAML formats between Hugo and Transifex, we use conversion scripts:
     - `scripts/i18n/tx_convert_push.py`: Converts Hugo YAML to Transifex format
     - `scripts/i18n/tx_convert_pull.py`: Converts Transifex YAML back to Hugo format
...
- **Script**: `themes/qgis-website-theme/assets/js/lang-redirect.js` (bundled via the header partial)
...
- **Update script**: `scripts/i18n/tx_fetch_coverage.py` (uses Transifex REST API v3)
