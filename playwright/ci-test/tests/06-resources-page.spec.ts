...
test('resources pages', async ({ page }) => {
  ...
  await expect(page.getByText('QGIS 3.28 QGIS 3.34 QGIS')).toBeVisible();
  await expect(page.getByText('QGIS 3.28', { exact: true })).toBeVisible();
  await expect(page.getByText('QGIS 3.34', { exact: true })).toBeVisible();
  await expect(page.locator('#tab-3')).toBeVisible();
  await expect(page.getByText('Archived reliases')).toBeVisible();
  await expect(page.getByText('For users(QGIS 3.28): Desktop')).toBeVisible();
  ...
  await expect(page.getByText('For documentation writers(QGIS 3.28):')).toBeVisible();
  await expect(page.getByText('For developers(QGIS 3.28):')).toBeVisible();
  ...
  await expect(page.locator('p').filter({ hasText: 'For download(QGIS 3.28):' })).toBeVisible();
  ...
  await expect(page.locator('table').filter({ hasText: 'Distribution Version QGIS version GRASS GIS version Fedora 36 3.28 8.0 37 3.28' }).locator('th').nth(1)).toBeVisible();
  ...
  await expect(page.getByText('QGIS Server 3.28 is currently')).toBeVisible();
  ...
  await expect(page.getByText('WMS 1.3.0 : http://test.qgis.')).toBeVisible();
  await expect(page.getByText('OGC API Features (aka WFS 3')).toBeVisible();
  await expect(page.getByText('WFS 1.1.0 : http://test.qgis.')).toBeVisible();
  await expect(page.getByText('Latest reports are WMS 1.3.0 and WFS')).toBeVisible();
  ...
  await expect(page.locator('#discover-qgis-3x---second-edition')).toContainText('Discover QGIS 3.x - Second Edition');
  ...
  await expect(page.getByText('QGIS.org, %%Y. QGIS 3.34. Geographic Information System Developers Manual. QGIS Association.').first()).toBeVisible();
  await expect(page.getByText('Cite the QGIS Installation')).toBeVisible();
  await expect(page.getByText('QGIS.org, %%Y. QGIS 3.34. Geographic Information System Installation Guide. QGIS Association.').first()).toBeVisible();
  ...
  await expect(page.getByText('QGIS.org, %%Y. QGIS 3.34. Geographic Information System User Guide. QGIS Association.')).toBeVisible();
  await expect(page.getByText('Cite the QGIS Server')).toBeVisible();
  await expect(page.getByText('QGIS.org, %%Y. QGIS 3.34. Geographic Information System API Documentation. QGIS Association.').first()).toBeVisible();
