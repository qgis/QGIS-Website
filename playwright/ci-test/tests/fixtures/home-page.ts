...
export class HomePage {
    ...
    constructor(public readonly page: Page) {
        ...
        this.freeOpenSourceSpatialDiv = this.page.locator('div').filter({ hasText: 'Free and open source Spatial' }).first();
        this.spatialWithoutCompromiseHeading = this.page.getByRole('heading', { name: 'Spatial without compromise' });
        this.createMapsHeading = this.page.getByRole('heading', { name: 'Create maps' });
        this.editLayersHeading = this.page.getByRole('heading', { name: 'Edit layers' });
        this.processAndAnalyzeHeading = this.page.getByRole('heading', { name: 'Process and analyze' });
        this.shareMapsHeading = this.page.getByRole('heading', { name: 'Share maps' });
        this.exploreQGISLink = this.page.getByRole('link', { name: 'Explore QGIS' });
        this.powerOfOpenSourceHeading = this.page.getByRole('heading', { name: '👋 The power of open source' });
        this.getInvolvedLink = this.page.getByRole('link', { name: 'Get involved' }).first();
        this.freeAndOpenSourceHeading = this.page.getByRole('heading', { name: 'Free and open source' });
        this.freeDownloadLink = this.page.getByRole('link', { name: 'Free download' });
        this.youTubeVideoThumbnailOverlayImage = this.page.frameLocator('iframe[title="YouTube video player"]').locator('.ytp-cued-thumbnail-overlay-image');
        this.startUsingQGISHeading = this.page.getByRole('heading', { name: 'Start using QGIS 🚀' });
        this.goToMaterialsLink = this.page.getByRole('link', { name: 'Go to materials' });
        this.qgisSupportersHeading = this.page.getByRole('heading', { name: 'QGIS supporters' });
        this.addYourLogoHereText = this.page.locator('div').filter({ hasText: 'Add your logo here?' }).nth(2);
        this.silverPartnerText = this.page.locator('div').filter({ hasText: 'Silver partner' }).nth(2);
        this.supportersGridDiv = this.page.locator('.supporters-grid > div:nth-child(3)').first();
        ...
        this.communityImg = this.page.locator('.explore > .columns > div > figure > img');
        this.changeLogVideo = this.page.frameLocator('iframe[title="YouTube video player"]').locator('.ytp-cued-thumbnail-overlay-image');
    }
