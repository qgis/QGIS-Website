...
---
# 実験・応用地理研究所の主要なGISソフトウェアとしてのQGIS
## 前書き
実験・応用地理研究所（LEAG）は、ブルノ、チェコ共和国でのホテルとビジネス経営学部の一部であり、株式会社の観光、ビジネス、ホテル経営で高い教育を提供しています（VysokáSKOLAobchodníAhotelová、SRO）。LEAGは、社会経済的、空間的プロセスをモデル化する空間パターンを分析し、地方及び地域の現実を記述および監視に焦点を当てています。
...
- グラフィカルユーザーインタフェース（商用製品に近いです）
- クイック開発およびユーザー欲しかった機能を統合
- 安定性と相互運用性（WindowsとUbuntu Linuxプラットフォームを使用しています）
- その他（HorstDüster博士による事例研究などで言及される）

空間解析は、基本的なジオプロセシング機能とPythonベースのプラグインを使用して主に行われます。QGISを使用した最近のプロジェクト：
- 空間的位置と住民との間のモデリングパターン
- 地形の可視化（および丘シャドウイングとしてさらに使用）のための3Dモデルを作成します
- 中小企業のデータベースの格納、作成および編集（+さらに可視化）
...
QGISは、他のオープンソース製品への相乗効果で使用されています。例えば、最終的なレイアウトは、ほとんどのInkscapeまたはGIMP（LibreOffice Drawは少ない）を使用して作成されます。Webベースの地図は、（SVGのエクスポート後）のInkscapeを使って編集しており、ラスター地図はGIMPを使用して事前に編集されています。
...
この記事は、ヤクブ・トロヤンによって2010年11月に寄稿されました。氏は、ビジネス・ホテル経営専門学校株式会社（ブルノ、CZ）での学術研究者で、実験・応用地理研究所を設立しリードしています。また、マサリク大学理学部地理研究所（ブルノ、CZ）で博士課程の学生です。
