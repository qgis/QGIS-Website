...
section: project
sidebar: true
title: QGIS 3.18の変更履歴
type: visual-changelog

---
{{<content-start >}}
# QGIS 3.18の変更履歴{#changelog318 }
...
パーセンタイルアルゴリズムは、入力小数値（0から1の範囲）に基づいてパーセンタイルを計算する可能性しか提供しませんが、パーセントランクラスタは入力値ラスターに基づいて計算することもできます。これにより、ユーザーは、データセット駆動の値入力（サンプルレイヤーなど）に基づいてセルごとのパーセントランクを計算する機会が得られます。
...
### 機能: 読み込み専用で生成されたフィールド
APIメソッドが追加され、読み込み専用で生成された列がエディタで見えないようになりました。

This feature was developed by [Julien Cabieces](https://api.github.com/users/troopa81)
### 機能: 定義済みパラメータを使ってMSSQLの読み込みを改善
レイヤーの読み込み時間を短縮するために、地図レイヤーの範囲を事前に計算し主キーを指定できるため、QGISでこれらの特性を計算する必要がなくなってパフォーマンスが向上します。
...
この機能は Metropole de Lille によって資金提供されました

This feature was developed by [Julien Cabieces](https://api.github.com/users/troopa81)
### Feature: Add advanced options for raster data imports
GDALデータプロバイダを使用してレイヤをインポートするときに高度なパラメータを指定できる新しい機能が導入されました。これにより、特に予想されるデータセットの問題や欠点を回避するときに、さまざまな新しい可能性が開かれます。
...
| ユーザーがINSERT権限しか持っていない場合、属性を入力してPostGISテーブルに編集を保存することはできません | [#40922](https://github.com/qgis/QGIS/issues/40922) | [PR #41059](https://github.com/qgis/QGIS/pull/41059) | いいえ |
| GeoPackage layer name truncated when filename contains more that one dot | [#40864](https://github.com/qgis/QGIS/issues/40864) | [PR #41061](https://github.com/qgis/QGIS/pull/41061) | いいえ |
| Unable to right click GeoPackage table from browser | [#40856](https://github.com/qgis/QGIS/issues/40856) | Works for me |  |
| Empty Temporary scratch layer made permanent is not editable | [#38834](https://github.com/qgis/QGIS/issues/38834) | [PR #41075](https://github.com/qgis/QGIS/pull/41075) | いいえ |
...
| Wrong WFS 2.0.0 request returns duplicate features | [#41087](https://github.com/qgis/QGIS/issues/41087) | [PR #41169](https://github.com/qgis/QGIS/pull/41169) | いいえ |
...
| QgsCompoundColorWidget instance crash when QgsApplication::colorSchemeRegistry() is empty \\ | [#28208](https://github.com/qgis/QGIS/issues/28208) | 無効 |  |
...
| Imported GeoTIFF not using data extent to select CRS - NAD83 vs NAD83(CSRS) | [#41456](https://github.com/qgis/QGIS/issues/41456) | 無効 |  |
...
| Filter is not applied in duplicated layers | [#40349](https://github.com/qgis/QGIS/issues/40349) | フィードバック |  |
