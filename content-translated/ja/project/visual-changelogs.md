---
HasBanner: false
draft: false
sidebar: true
title: ビジュアル変更履歴
type: page

---
{{<content-start >}}
## ビジュアル変更履歴
QGISはコミュニティによる取り組みです。このリリースを実現するために時間と労力をボランティアで提供してくださった（または資金援助をしてくださった）開発者・ドキュメント作成者・テスター・その他多くの方々に心より感謝申し上げます。QGISコミュニティより、このリリースをお楽しみいただければ幸いです！QGISをさらに素晴らしいものにするために、時間・資金・その他の形でご貢献いただける方は、ぜひ [QGIS.ORG](https://qgis.org) をご覧いただき、お力をお貸しください！

QGISは寄付者とサステイニングメンバーによって支えられています。プロジェクトに大小問わず財政的な寄付をしてくださった寄付者の現在のリストは、 [寄付者一覧](/funding/donate/donors/) でご覧いただけます。プロジェクトの公式サステイニングメンバーになりたい方は、詳細について [サステイニングメンバーのページ](/funding/membership/)  をご覧ください。QGISへの資金援助は、定期的な開発者ミーティングの開催、プロジェクトインフラの維持、バグ修正活動の資金として活用されます。最新のスポンサーの完全なリストを以下に掲載します。すべてのスポンサーの皆さまに心より感謝申し上げます！

QGIS はフリーのソフトウェアです。利用する上で金銭を支払う義務はありません。実際、私たちは利用者の財政状況や社会的地位にかかわらず、より多くの人に QGIS を利用して欲しいと思っています。私たちは地理空間的な意思決定ツールによって人々が力を持つことが、全人類の社会をより良くすると信じています。

以下に、バージョン2.0以降のビジュアルチェンジログを掲載しています。

{{<visualchangelogs >}}

{{<content-end >}}
