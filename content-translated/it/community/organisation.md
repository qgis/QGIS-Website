---
HasBanner: false
draft: false
sidebar: true
title: Organizzazione del progetto
type: page

---
{{<content-start >}}
# Who's Who
QGIS come Progetto è correntemente organizzato come associazione con sede legale in Svizzera, con la seguente struttura:

![QGIS Organizational Structure](./QGIS-ORG_Organizational_structure.png)

Struttura Organizzativa di QGIS

Il Progetto QGIS è gestito dal Board (3 membri, rappresentazione legale) e dal PSC composto da 6 membri (con le rispettive responsabilità indicate tra parentesi):

Consiglio
-
* Marco Bernasocchi (Chair) \[dal 5.2020\]
* Anita Graser (Vice chair) \[dal 9.2022\]
* Andreas Neumann (Tesoriere; Referente per i Membri Sostenitori) \[da 11.2015\]

Per contattare il Board, si prega di scrivere a [board@qgis.org](mailto:board@qgis.org).

Comitato direttivo del progetto
-
* Marco Bernasocchi (Chair) \[Chair dal 5.2020; Vice chair 4.2018 - 4.2020\]
* Anita Graser (Vice chair) \[dal 9.2013; Vice chair dal 9.2022\]
* Andreas Neumann (Tesoriere; Referente per i Membri Sostenitori) \[da 11.2015\]
* Jürgen Fischer (Release Manager) \[dal 9.2013\]
* Alessandro Pasotti (PSC Member) \[Vice chair 5.2020 - 9.2022\]
* Régis Haubourg (PSC Member) \[dal 10.2022\]

Per contattare il PSC, scrivere a [psc@qgis.org](mailto:psc@qgis.org).
### Membri onorari del PSC
* Gary Sherman (Fondatore e Membro onorario a vita del PSC)
* Tim Sutton (nominato Membro onorario del PSC nel settembre 2018 al FOSS4G in Tanzania)

### Ex membri del PSC
* Tim Sutton (Chair; Testing/QA Manager) (sino al 2018)
* Paolo Cavallini \[Member 4.2020 - 9.2022; Chair 4.2018 - 4.2020; Vice chair 1.2008 - 3.2018\]
* Marco Hugentobler \[sino al 2016\]
* Otto Dassau (Community Advisor) \[2007 - 2016\]
* Richard Duivenvoorde (Infrastructure Manager) \[2013 - 2019\]

Per ulteriori informazioni sulle responsabilità del PSC, consultare [qui]({{< ref "/community/foundation/charter.md" >}}).

Indirizzo dell'Organizzazione

QGIS.ORG Via Geinas 2, CH-7031 Laax
## Membri con diritto di voto
### Membri con diritto di voto onorari
* Harrissou Sant-anna \[dal 5.2020\]
* Nyall Dawson \[dal 5.2020\]

### Membri votanti
In base al nostro statuto, ogni gruppo di utenti che viene costituito seleziona un membro votante del gruppo di utenti QGIS che ne rappresenti gli interessi. L'elenco dei membri votanti è disponibile [qui]({{< ref "/community/groups.md" >}}). Se avete registrato un nuovo gruppo di utenti o avete cambiato il rappresentante del vostro gruppo, inviate un'e-mail all'indirizzo [trademark@qgis.org](mailto:trademark@qgis.org).
### Membri con diritto di voto della Comunità
In base ai nostri statuti, per ogni membro votante può essere eletto un membro della comunità più ampia. Il seguente elenco include i membri votanti correnti della comunità:
* Martin Dobias, Slovacchia
* Nathan Woodrow, Australia
* Matthias Kuhn, Svizzera
* Larry Shaffer, Stati Uniti
* ~~Nyall Dawson, Australia~~ (moved to Honorable Voting Member status in 2020)
* Giovanni Manghi, Portogallo
* Denis Rouzaud, Svizzera
* Alexander Bruy, Ucraina
* Regis Haubourg, Francia
* Even Rouault, Francia
* Werner Macho, Austria
* Vincent Picavet, Francia
* Yves Jacolin, Francia

Aggiunti nel 2017
* Alessandro Pasotti, Italia
* Victor Olaya, Francia/Spagna
* Lene Fischer, Danimarca
* Paul Blottiere, Francia
* Mathieu Pellerin, Indonesia
* Luigi Pirelli, Spagna
* Alexandre Neto, Portogallo
* Tom Chadwin, United Kingdom
* Marco Hugentobler, Svizzera

Aggiunti nel 2018
* Hugo Mercier, Francia

Aggiunti nel 2019
* Nessuno (nessun nuovo gruppo di utenti creato)

Aggiunti nel 2020
* David Signer, Switzerland
* Enrico Ferreguti, Italia
* Etienne Trimaille, Francia
* Pedro Venâncio, Portogallo
* Peter Petrik, Repubblica Ceca
* Richard Duivenvoorde, Olanda

Aggiunti nel 2021
* Andrea Giudiceandrea, Italia

Aggiunti nel 2022
* Adelcides Varela, Capo Verde
* Germán Carrillo, Colombia

Added 2024
* Stefanos Natsis, Grecia
* Isabel Kiefer, Svizzera
* Damiano Lombardi, Svizzera

Added 2026
* Lova Andriarimalala, Madagascar

### Membro con diritto di voto di OSGEO
* Michael Smith [dal 2015/16]

## Gestione delle release
Attuale Responsabile delle Release: Jürgen E. Fischer

I rilasci di QGIS sono gestiti dal Release Team (Release Manager e Release Assistants). Insieme a numerosi collaboratori, essi cercano di garantire rilasci puntuali, regolari e stabili di QGIS sulle varie piattaforme supportate. Il Release Team recluta e aiuta volontari a creare pacchetti per le varie piattaforme supportate e gestiscono il sistema di bug tracking.
### Packaging Team
QGIS viene pacchettizzato per diversi tipi di sistemi operativi. Le persone responsabili di questi pacchetti sono le seguenti:
* Windows Team: Jürgen E. Fischer
* MacOS Team: Matthias Kuhn, OPENGIS.ch
* Team Linux:
  * Responsabile RPM OpenSuSE: Angelos Tzotsos
  * Fedora RPM Maintainer: Daniel Viganò
  * Debian / Ubuntu Deb Maintainer: Jürgen E. Fischer, Bas Couwenberg

### Team di Collaudo
* Ubuntu Linux: Ricardo Soares Garcia, Giovanni Manghi, Luís de Sousa
* Debian Linux: Paolo Cavallini, Pedro Venâncio, Ivan Mincik
* Windows: Giovanni Manghi, Pedro Venâncio, Duarte Carreira, Andre Mano, Rui Pedro Henriques
* macOS: Denis Rouzaud, Matthias Kuhn, Isabel Kiefer

### Team Sicurezza
Per domande di sicurezza, si prega di scrivere a [security@qgis.org](mailto:security@qgis.org).

Risorse della comunità
-
Le risorse della comunità QGIS sono gestite dal Community Team (Community Manager e Assistenti). Con numerosi contributori mantengono i contatti con la comunità, si occupano degli eventi QGIS, della documentazione, delle traduzioni della GUI, e garantiscono il corretto funzionamento dell'esperienza online di QGIS.org, inclusi siti web, blog e mailing list.

Current Community Team Leaders:
* Lene Fischer (community tasks)
* Matteo Ghetta (documentazione)

Find the list of documentation and website translators at: [https://github.com/qgis/QGIS-Documentation](https://github.com/qgis/QGIS-Documentation)
### Traduzione
Current Translation Team Lead: Werner Macho

The QGIS Translation is managed by the Community Assistant (Translation Team Lead) and supported by additional language specific teams. Information about the current documentation status and the translation process is provided at the [Translators Corner](https://docs.qgis.org/latest/en/docs/documentation_guidelines/do_translations.html).

Contact : [qgis-community-team](http://lists.osgeo.org/mailman/listinfo/qgis-community-team) list or [QGIS Translation Mailing list](http://lists.osgeo.org/mailman/listinfo/qgis-tr)
### Materiale per la stampa + Traduzione
Current Press Material Team Lead: Anita Graser

QGIS Press Material and its translations are managed by the Community Assistant (Press Material Team Lead) and supported by additional language specific teams. Information about current activities and the translation process is provided at the Press Centre.

TEAM
-
Un progetto di successo richiede molta gente che ricoprono ruoli diversi. Alcuni scrivono codice o documentazione, mentre altri fanno un prezioso lavoro come testing, inoltro di  patch e suggerimenti.

The number of Contributors to the project is unbounded. Get involved today. All contributions (adding code or documentation, fixing typo error, reporting issues…) to the project are greatly appreciated.

These [individuals](https://github.com/qgis/QGIS/graphs/contributors) are recognised as Contributors to the main code [github repository](https://github.com/qgis/QGIS).

Follow the lists of maintainers of the [documentation](https://github.com/qgis/QGIS-Documentation/graphs/contributors) and the [website](https://github.com/qgis/QGIS-Website/graphs/contributors). Feel free to join any of those teams.

{{<content-end >}}
