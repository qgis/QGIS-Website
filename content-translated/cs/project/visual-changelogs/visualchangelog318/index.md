...
### Funkce: Přiblížení a posun na výběr pro více vrstev
...
Byly přidány nové možnosti exportu pro síťové vrstvy. Ty umožňují následující možnosti při exportu sítě do vektorové vrstvy:
- Export faces
...
Předchozí verze QGIS vyžadovaly použití zásuvného modulu crayfish, tato funkce však byla nyní začleněna do jádra QGIS a umožňuje uživatelům exportovat vrstvy sítě do vektorů a přidat hodnotu datové sady jako atribut.
...
Z pluginu crayfish byla přenesena řada nových algoritmů zpracování, včetně:
...
Barvy 3D materiálů lze nyní definovat pomocí dat, což umožňuje určit barvy prvků pomocí hodnoty atributu nebo výrazu.
...
Texturované materiály nejsou v tuto chvíli podporovány.
...
This feature was developed by [Mathieu Pellerin](https://api.github.com/users/nirvn)
## Point Clouds
### Funkce: Podpora bodových mračen
...
Součástí jsou popisky pro minimální a maximální hodnotu rampy, které se vykreslují pomocí stejného nastavení písma jako text ostatních položek legendy:
...
- Ramp direction
- Přizpůsobení formátování textu
...
This feature was developed by [Nyall Dawson](https://api.github.com/users/nyalldawson)
### Funkce: Dynamické předvolby textu
...
This feature was developed by [Nyall Dawson](https://api.github.com/users/nyalldawson)
## Výrazy
### Funkce: Volitelné formátování výsledků UUID
...
### Funkce: Podpora funkcí min, max, majority, sum, mean a median pro číselná pole
...
Nyní je možné vybírat prvky pomocí kontextové nabídky na plátně mapy. Kliknutí pravým tlačítkem myši na mapě inteligentně identifikuje prvky pod kurzorem myši a umožní přidat nebo odebrat dostupné prvky z aktuálního výběru. Pokud je k dispozici více prvků, může přetrvávat krátká prodleva, zatímco se systém pokusí vyhledat dostupné prvky, které se zpřístupní v položce vnořené nabídky pro výběr, nebo mohou uživatelé jednoduše použít funkci *vybrat vše* pro výběr všech dostupných prvků na dané pozici.
...
This feature was developed by [Nyall Dawson](https://api.github.com/users/nyalldawson)
## Datový Management
### Funkce: Nový algoritmus exportu do tabulkového procesoru
...
This feature was developed by [Mathieu Pellerin](https://api.github.com/users/nirvn)
## Zpracování
### Funkce: Povolení výrazu pro pole pořadí v algoritmu PointsToPath
...
### Funkce: Přepsání CRS pro výstup Clip Raster podle rozsahu
...
Vlastnosti nástroje **Statistika buněk** byly zopakovány a upraveny tak, aby umožňovaly tři nové algoritmy zpracování, které umožňují QGIS vypočítat statistiku buněk podobně jako běžné procentuální funkce používané v tabulkových procesorech. Nové algoritmy zpracování jsou:
...
### Funkce: Algoritmus zpracování bodů na čáry
Byl přidán nativní algoritmus zpracování v jazyce c++, který vytváří vrstvy čar z vrstev bodů, podobně jako stávající pythonovský *Points to path*, ale s rozšířenou funkčností:
- Added support for *natural sorting* ( \'10\' \> \'9\' )
- Vyhněte se vytváření neplatných čar s jedním vrcholem
...
Jedná se o přechodné nastavení, které je zapomenuto, jakmile je QGIS uzavřen.

![image42](images/entries/0115a38666f35166fa9e6fd53791a7bc8f73b6dc.webp)

This feature was developed by [Nyall Dawson](https://api.github.com/users/nyalldawson)
## Prohlížeč
### Funkce: Sjednocení připojení ArcGis Feature Service a ArcGIS Map Service v prohlížeči
...
### Funkce: Další typy geometrie pro export PostGIS
...
QGIS nyní podporuje databáze SAP HANA. SAP HANA je in-memory databáze s prostorovým enginem kompatibilním s OGC, jejíž expresní verze je zdarma k dispozici na adrese www.sap.com.

This feature was developed by [Maxim Rylov](https://api.github.com/users/mrylov)
### Funkce: Zrušení podpory DB2
**Zprostředkovatel dat DB2 byl označen jako zastaralý** a ve výchozím nastavení je v uživatelském rozhraní QGIS skrytý.
...
This feature was developed by [Julien Cabieces](https://api.github.com/users/troopa81)
### Funkce: Přidání pokročilých možností pro import rastrových dat
...
Tuto funkci vyvinul Mathieu Pellerin
## QGIS Server
...
Konzola QGIS Python nyní podporuje spuštění více vybraných příkazů z dialogu historie přidáním nového tlačítka **Vložit**.
...
| Název chyby | URL issues.qgis.org (if reported) | URL Commit (Github) | 3.16 backport commit (GitHub) |
| --- | --- | --- | --- |
| Crash with VRT layers using Python pixel functions | [#41048](https://github.com/qgis/QGIS/issues/41048) | Works for me |  |
| Cannot enter attributes and save edits on PostGIS table when user has only the INSERT privilige | [#40922](https://github.com/qgis/QGIS/issues/40922) | [PR #41059](https://github.com/qgis/QGIS/pull/41059) | ne |
| GeoPackage layer name truncated when filename contains more that one dot | [#40864](https://github.com/qgis/QGIS/issues/40864) | [PR #41061](https://github.com/qgis/QGIS/pull/41061) | ne |
| Unable to right click GeoPackage table from browser | [#40856](https://github.com/qgis/QGIS/issues/40856) | Works for me |  |
| Empty Temporary scratch layer made permanent is not editable | [#38834](https://github.com/qgis/QGIS/issues/38834) | [PR #41075](https://github.com/qgis/QGIS/pull/41075) | ne |
| Update MXE build, several issues | více problémů | [Commit afff8ba7be2794fe521a462feaf83b1a5d7cce63](https://github.com/qgis/QGIS/commit/afff8ba7be2794fe521a462feaf83b1a5d7cce63) [PR #40923](https://github.com/qgis/QGIS/pull/40923) | yes (CI) |
...
| Chybný požadavek na WFS 2.0.0 vrací duplicitní funkce | [#41087](https://github.com/qgis/QGIS/issues/41087) | [PR #41169](https://github.com/qgis/QGIS/pull/41169) | ne |
...
| QGIS Server WFS 1.1: problém s uspořádáním os | [#36584](https://github.com/qgis/QGIS/issues/36584) | [PR #41242](https://github.com/qgis/QGIS/pull/41242) | [PR #41608](https://github.com/qgis/QGIS/pull/41608) |
...
| Nelze vytvořit nové tabulky v geografickém balíčku (prohlížeč) | [#41342](https://github.com/qgis/QGIS/issues/41342) | [PR #41355](https://github.com/qgis/QGIS/pull/41355) | [PR #41403](https://github.com/qgis/QGIS/pull/41403) |
...
| initGUI() results in Segmentation fault on macOS in standalone PyQgis | [#40378](https://github.com/qgis/QGIS/issues/40378) | odpověděl s opravou skriptu |  |
| Pád instance QgsCompoundColorWidget, když je funkce QgsApplication::colorSchemeRegistry() prázdná \\ | [#28208](https://github.com/qgis/QGIS/issues/28208) | neplatné |  |
...
| Data-defined setting in html frame does not work | [#41590](https://github.com/qgis/QGIS/issues/41590) | provedeno šetření |  |
...
| Imported GeoTIFF not using data extent to select CRS - NAD83 vs NAD83(CSRS) | [#41456](https://github.com/qgis/QGIS/issues/41456) | neplatné |  |
| Georeferencer: oprava pádu souborů .points generovaných staršími verzemi QGIS | nehlášeno | [PR #41480](https://github.com/qgis/QGIS/pull/41480) | N/A |
...
| Název chyby | URL issues.qgis.org (if reported) | URL Commit (Github) | 3.16 backport commit (GitHub) |
| --- | --- | --- | --- |
| Record count regression on views | [#41188](https://github.com/qgis/QGIS/issues/41188) | Neopraví |  |
...
| Název chyby | URL issues.qgis.org (if reported) | URL Commit (Github) | 3.16 backport commit (GitHub) |
| --- | --- | --- | --- |
| Fix text edit widget wrapper incorrectly shows \"NULL\" string for indeterminate state | nehlášeno | [Commit fa91357](https://github.com/qgis/QGIS/commit/fa9135792574ff5dbbd1ab53d15faec3d190e3db) | [PR #41054](https://github.com/qgis/QGIS/pull/41054) |
| \[layouts\] Expose the correct expression context for attribute and manual table text format widgets | nehlášeno | [Commit f337271](https://github.com/qgis/QGIS/commit/f3372716060f2c1eecff46c99c999b66505504fc) | [PR #41054](https://github.com/qgis/QGIS/pull/41054) |
| Fix incorrect error when saving attribute modifications to layers with joins | nehlášeno | [Commit 01889d9](https://github.com/qgis/QGIS/commit/01889d96b22757028e62c156a945ae8b86e69a68) | [PR #41054](https://github.com/qgis/QGIS/pull/41054) |
| Fix misleading JSON error message when an attribute form fails to save changes | nehlášeno | [Commit fd657be](https://github.com/qgis/QGIS/commit/fd657bee96c2b58c035a69a71fd58d1cb409770e) | [PR #41054](https://github.com/qgis/QGIS/pull/41054) |
| Fix project \"avoid tile artifacts\" setting has no effect on server | [#37679](https://github.com/qgis/QGIS/issues/37679) | [Commit 04d6ea9](https://github.com/qgis/QGIS/commit/04d6ea99b43fb8e426dc1cb31b2b7e913aca203c) | Nevhodné |
| Don\'t crash when a relation is created where the field names defined in the relation differ in case from the layer\'s field names | nehlášeno | [Commit 0f49cb9](https://github.com/qgis/QGIS/commit/0f49cb909732882e4c0b3ba26a6987fff95e29ea) | [Commit c708ca58](https://github.com/qgis/QGIS/commit/c708ca58e81734abed0193d596a451609edd295a) |
| Oprava tlačítek přidat/odebrat nemají ikony v dialogových oknech pro vytvoření vztahu | nehlášeno | [Commit 2e81dbc](https://github.com/qgis/QGIS/commit/2e81dbcc50e510afc9033be9a99bfcb2512f6fd9) | N/A |
| Fix restoring previous GDAL alg raster creation options when running an algorithm from processing history | nehlášeno | [Commit 645397e](https://github.com/qgis/QGIS/commit/645397e83d88c124e8ca8faa46b304c2e74ec3e9) | [Commit c708ca5](https://github.com/qgis/QGIS/commit/c708ca58e81734abed0193d596a451609edd295a) |
...
| Fix crash when running model from file through qgis_process with the \--json switch | nehlášeno | [Commit ccd4fb6](https://github.com/qgis/QGIS/commit/ccd4fb67eb309a077e959df60c0d3556321453da) | [Commit 8bda37c](https://github.com/qgis/QGIS/commit/8bda37ca0f2b914fa5fe9916398a2d08c1ef774d) |
| When labelling job takes a long time to render, don\'t clear the map canvas between the finished layer render state and final state | nehlášeno | [Commit 1beb1bd](https://github.com/qgis/QGIS/commit/1beb1bdc788ba64f826c54d3ab587fcc5289afb4) | N/A |
...
| Fix incorrect feature request filter expression generated when rule based renderer uses a NOT IN rule | nehlášeno | [Commit a895598](https://github.com/qgis/QGIS/commit/a89559879743242d2b7138c2d3d73179c380af97) | [Commit 5eec9e4](https://github.com/qgis/QGIS/commit/5eec9e42f3e4ddeb17f63a194df7a1f9ccb2e5e5) |
| Ask users for existing destination fields for Sum Line Length and Count Points In Polygons result fields when running in in-place mode | [#39807](https://github.com/qgis/QGIS/issues/39807) | [Commit 12e8130](https://github.com/qgis/QGIS/commit/12e8130beb0836fe18bf741f5ad26bf599feb84f) | Nevhodné |
| Oprava pádu, když v projektu neexistují žádné vrstvy | nehlášeno | [Commit 039bc20](https://github.com/qgis/QGIS/commit/039bc202733d57e5daaa13d417eb78b2c072851d) | N/A |
| \[processing\] Fix exception in algorithm locator filter if an algorithm has no group set | nehlášeno | [Commit 46dbd8b](https://github.com/qgis/QGIS/commit/46dbd8b2d6275256779c14938de7424ab1053d7a) | [Commit 7ef7b17](https://github.com/qgis/QGIS/commit/7ef7b17f2601bd8ee8f31dc5be7a69b90c89550f) |
| Expose additional GDAL supported resampling methods as options for \"early\" raster resampling | [#40746](https://github.com/qgis/QGIS/issues/40746) | [Commit 13fbcd1c](https://github.com/qgis/QGIS/commit/13fbcd1cff3dfa31a7c592566de9d2f5be1cf965) | Nevhodné |
...
| Oprava průhlednosti pro vrstvy ImageServeru přidané přes prohlížeč | [#41126](https://github.com/qgis/QGIS/issues/41126) | [Commit 9fb554f](https://github.com/qgis/QGIS/commit/9fb554fde0348c3a78aad6070c6c83986e5470ab) | Nevhodné |
| \[server\] Just skip any broken projects on the landing page, instead of blocking the whole landing pag | nehlášeno | [Commit e4a4b83](https://github.com/qgis/QGIS/commit/e4a4b832dff47e552271c8b024eb8105b88483cb) | Nevhodné |
| Automatically update map canvas and layouts if set to a user crs and the user changes that crs definition | nehlášeno | [Commit ea57cfe](https://github.com/qgis/QGIS/commit/ea57cfea1ff4320b2bd6b7e6be5aea699bdeabe3) | Nevhodné |
| Don\'t show duplicate \"source\"/\"path\" rows for local files | nehlášeno | [Commit 8aa9c6a](https://github.com/qgis/QGIS/commit/8aa9c6a142523aed85ce74d70b93c04553e13dc0) | N/A |
...
| Fix fill symbol only related properties (like ring filters) show for line symbols in some contexts. | [#33398](https://github.com/qgis/QGIS/issues/33398) | [Commit 9cba248](https://github.com/qgis/QGIS/commit/9cba2489bcde55ee6e072dc7690ff2401928c507) | Nevhodné |
...
| Oprava chybějících položek nabídky identifikace výsledků pravým tlačítkem myši pro jiné než vektorové typy vrstev | nehlášeno | [Commit 176e972](https://github.com/qgis/QGIS/commit/176e9728c5ebb24f309369878664c2c49dc6eb16) | Nevhodné |
