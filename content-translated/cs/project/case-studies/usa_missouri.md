...
title: QGIS Mapy historických záznamů o herpetofauně (v Missouri, USA)
type: case-study

---
{{<content-start >}}
# QGIS Mapy historických záznamů o herpetofauně (v Missouri, USA)
...
## Vytváření a export statických map
Proces vytváření map pomocí QGIS začíná s daty uloženými v několika tabulkách PostgreSQL, které jsou prostorově upraveny pomocí PostGIS. Protože mapy jsou statické a v konečném důsledku jsou určeny buď pro web, nebo pro tištěný atlas, jsou pro každý druh generovány automaticky pomocí vlastního pluginu QGIS Python (obr. 1,2).
...
## Závěry
Na začátku projektu MOHAP bylo k ukládání a zpracování dat a tvorbě map pro publikaci použito několik komerčních a proprietárních nástrojů. Od roku 2007 jsme se rozhodli převést všechny aspekty projektu na software s otevřeným zdrojovým kódem. QGIS spolu s PHP, PostgreSQL, PostGIS, Pythonem a ReportLabem tvoří open source základ projektu MOHAP, který fakticky umožňuje, aby projekt a všechna data existovaly bez proprietárního softwaru.

QGIS obsahuje nativní podporu PostGIS a architekturu zásuvných modulů Python, které byly nezbytné pro vytvoření automatizovaného generování a exportu map. Rozsáhlá dokumentace API byla spolu s kuchařkou pro vývojáře zásuvných modulů použita k vytvoření přesně toho, co jsme pro automatizaci potřebovali. Podpora komunity je také velmi dobrá a zahrnuje obrovské množství sdílených zásuvných modulů vytvořených a připravených k použití.

Přestože QGIS používáme v malém a specifickém rozsahu, jeho možnosti a rozšiřitelnost pomocí Pythonu jsou více než dostatečné pro řešení větších a složitějších projektů.
## Autor
<figure>
