...
## Jak byl QGIS použit
V rámci celého procesu plánování byl QGIS použit pro následující čtyři úkoly:
- Import stávajících dat. Data zahrnují georeferencované podkladové mapy (\*.tif) spolu se soubory polygonů, linií a bodů. Pro lesního hospodáře je vizualizace existujících tematických dat (například zdrojů podzemní a pitné vody, zón ochrany přírody) důležitá pro jeho každodenní práci.
...
- Analýza. Byly vypočteny plochy jednotlivých kategorií měření. Od určitých liniových prvků, jako jsou železniční tratě nebo silnice, byly pomocí nárazníku určeny odpovídající ochranné lesy.
- Tisk a export. Tiskový nástroj podporuje tvorbu rozvržení mapy. Mapa byla exportována do formátu PDF a obrázkových souborů.
...
## Perspektiva
Základní úkoly koncepce lesnického plánování lze zpracovat pomocí uvedených základních nástrojů GIS. Guaraci bude i nadále používat QGIS pro lesnické plánování na úrovni vlastníků lesa. Na základě zkušeností s předloženým projektem lze QGIS doporučit i pro lesnické koncepce na úrovni místních úřadů. Větší a složitější projekty s požadavky na podrobnou analýzu by mohly být řešeny kombinací QGIS s prostorovým databázovým serverem Postgis. Nové těžební zóny i lesní rezervace a ochranná pásma lze snadno digitalizovat. Doufejme, že vlastníci a správci lesů budou stále více využívat potenciál QGIS také pro své roční plánování i dokumentaci lesnických činností.
## Autor
<figure>
