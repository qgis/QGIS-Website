...
title: O uso da consola python para preparar um mapa offline a partir de um WMS (transferência
  de mosaicos)
type: case-study
...
Este estudo explica como preparar mapas offline a partir de WMS/WFS ou qualquer mapa carregado na vista de mapa QGIS através da transferência de mosaicos com ajuda de um script na consola python. O script também gera um único mosaico a partir de todos para uso futuro. 

Aceder a mapas raster/vetor através de Serviços de Mapas Web (WMS) é muito comum e eficiente. Exemplos disso são, o Google Maps, Yahoo, Bing, MODIS, Landsat, e mapas publicados pela NASA, NRSC (Índia). A desvantagem/limitação do WMS é trabalhar pela internet ou intranet. Pode não ser possível obter uma ligação à internet especialmente durante levantamentos no campo. Outra coisa, é o WMS trabalhar lentamente, comparativamente a outro formato de offline tais como, GeoTiff, jpg, img, etc.
## Utilização do QGIS
...
O método tem uma limitação, quando altera o mapa no servidor acabará por se refletir offline (mapa transferido). Neste caso o utilizador tem de repetir o processo e gerar um novo mapa offline. A segunda limitação é a que o mapa criado será um formato de imagem; não pode ser usado para mapas temáticos ou classificação de imagens. Com alguns esforços extra alguns mapas de uso do solo podem ser convertidos em vetor sem perda de dados.
## Âmbito
O mosaico gerado é uma imagem pura mesmo se for transferida a partir de uma mapa temático (7-8 cores). Usando uma ferramenta avançada como a reclassificação, camada/banda guardada no raster pode ser convertida num vetor poligonal. O WMS tem vários pedidos tais como, getcapabilities, getmap, getfeatureinfo, etc. Os polígonos podem ser recuperados usando a informação do atributo getfeatureinfo. O script pode ser convertido num módulo para que qualquer pessoa sem conhecimentos de programação possam usar o script.
## Conclusões
...
## Autores
Este artigo foi contribuído em Maio 2014 pelo Prashant Kadgi, que vive em Pune (Índia) e atualmente trabalha como consultor na GIS APP Consultancy Services. Desde 2009, ele usa SIG & DR em projetos NRM, floresta, e-governance e software em diferentes regiões da Índia e do mundo. 
