...
section: project
sidebar: true
title: Список изменений в QGIS 3.12
type: visual-changelog

---
{{<content-start >}}
# Список изменений в QGIS 3.12{#changelog312 }
![image1](images/projects/dbf38cfe3b603e7e0b840336d39b399a1a46e52d.png)

Release date: 2020-02-21

Если вы ждали новые функции и исправления множества ошибок, выход этой версии вас очень обрадует. QGIS 3.12 добавляет множество новых возможностей почти к каждой части QGIS. Начиная с  масок подписей , встроенному провайдеру PG Raster, потрясающей новой возможностью использовать сетевые (mesh) слои и многим-многим другим - эта версия порадует чем-то каждого. Как всегда, мы хотели бы напомнить вам, что QGIS является проектом с открытым исходным кодом и, если вы имеете такую возможность, окажите поддержку нашей работе посредством пожертвований, спонсорства или вклада в документацию по коду, веб-сайт и т. д.
...
{{<table-of-contents >}}
## Инструменты
### Функциональность: QgsMapLayerAction: добавлен пункт "Действие при выделении" (action on selection) в контекстном меню слоя 
В контекстное меню слоя добавлен новый пункт (Действия при выборе - Actions on selection). Пункт отображается только при необходимости (у слоя есть выделение, и на данном слое можно выполнять действия).
...
### Функциональность: Отображение расстояния от местоположения по GPS до текущей позиции курсора
Когда присоединён GPS-приёмник, всякий раз, когда пользователь перемещает курсор в окне карты в строке состояния отображаются расстояние и азимут до позиции, определённой приёмником GPS.

Для расстояния учитывается проекция и настройки азимута.
...
### Функциональность: Добавлена настройка проекта для показа единиц измерения азимута
Вкладка Оцифровка меню Установки - Параметры содержит новую настройку для контроля формата используемого по-умолчанию для отображения единиц измерения азимута для вновь создаваемых проектов. Каждый раз при создании нового проекта, будут применяться эти настройки по-умолчанию.
...
| unreported: wrong link in server WFS3 items page (too many slashes) | не зарегистрировано | [PR #33926](https://github.com/qgis/QGIS/pull/33926) | risky |
...
| DB Manager in 3.11 Master can\'t connect to PostGIS Enabled database | [#34132](https://github.com/qgis/QGIS/issues/34132) | [PR #34171](https://github.com/qgis/QGIS/pull/34171) | н/д |
...
| Fields are shifted when importing a layer with an FID attribute into GeoPackage | [#32927](https://github.com/qgis/QGIS/issues/32927) | [PR #32934](https://github.com/qgis/QGIS/pull/32934) | н/д |
...
| Runs of testsuite leaves hundreds of directories in \~/.local/share | [#34185](https://github.com/qgis/QGIS/issues/34185) | [Commit 96a7fb8](https://github.com/qgis/QGIS/commit/96a7fb8f5acf3a278919f88c7c5abc4d20b8b6c2) | н/д |
| Drop use of deprecated QgsCoordinateReferenceSystem constructor | [PR #34186](https://github.com/qgis/QGIS/pull/34186) | [Commit b4fa419](https://github.com/qgis/QGIS/commit/b4fa419f7654ea7d1bce666b99f0534c415d8634) | н/д |
| Only look for pointcloud in its installed extension schema | [#33509](https://github.com/qgis/QGIS/issues/33509) | [Commit 1f44b29](https://github.com/qgis/QGIS/commit/1f44b29933bff3b8806bba75b761dd47b6c349c8) | н/д |
...
| QGIS WFS / OGC API -- Features not showing list typed attributes correctly | [#33758](https://github.com/qgis/QGIS/issues/33758) | [PR #33983](https://github.com/qgis/QGIS/pull/33983) | н/д |
...
| Название ошибки | URL адрес на issues.qgis.org (если было сообщено) | URL адрес Коммита (Github) | Коммит обратного порта для 3.10  (GitHub) |
| --- | --- | --- | --- |
| Snapping and vertex tool not working with some layers if snapping was enabled when loaded | [#33449](https://github.com/qgis/QGIS/issues/33449) | [PR #33757](https://github.com/qgis/QGIS/pull/33757) | н/д |
| Snapping not possible in new project with new layer | [#32969](https://github.com/qgis/QGIS/issues/32969) | Дублирование | - |
...
| Rubberband not working correctly with \"Shapes\" digtizing in rotated map window | [#33287](https://github.com/qgis/QGIS/issues/33287) | Дублирование |  |
...
| Fix identify menu when an expression is used as display name | не зарегистрировано | [PR #34361](https://github.com/qgis/QGIS/pull/34361) | [PR #34402](https://github.com/qgis/QGIS/pull/34402) |
...
| Oracle: таблицы перечислены один раз для полигонов и один раз для мультиполигонов, но обе показывают всю геометрию | [#32521](https://github.com/qgis/QGIS/issues/32521) | [PR #34358](https://github.com/qgis/QGIS/pull/34358) |  |
| BUG: fix invalid layer result when Oracle version is under 12 | не зарегистрировано | [PR #34546](https://github.com/qgis/QGIS/pull/34546) |  |
