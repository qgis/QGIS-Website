...
section: project
sidebar: true
title: Список изменений в QGIS 3.14
type: visual-changelog

---
{{<content-start >}}
# Список изменений в QGIS 3.14{#changelog314 }
...
QGIS является свободным программным обеспечением, и вы не обязаны платить за его использование - на самом деле, мы хотим, чтобы люди повсюду использовали его независимо от финансового или социального статуса - мы верим, что расширение прав и возможностей людей с помощью пространственных инструментов принятия  решений приведет к улучшению общества для всего человечества.

{{<table-of-contents >}}
## Общие
### Новые настройки подписей для Сетки Оформления
...
Спонсорская поддержка - iMHere Asia

This feature was developed by [Mathieu Pellerin](https://api.github.com/users/nirvn)
## Временны́е данные
### Настройка глобального временного диапазона в панели контроля времени
В настройках времени добавлен параметр, позволяющий глобально установить единый временной диапазон анимации. Это означает, что все кадры анимации будут иметь одинаковую дату и время начала, но разные даты и время окончания.
...
В этом режиме, слой будет автоматически перерисовываться при каждом изменении временного диапазона, но фильтрация по времени к объектам применяться не будет.

Эта конфигурация полезна, когда для слоя установлены настройки символики, которые варьируются в зависимости от временного диапазона. Например, когда слой использует зависящие от времени выражения рендерера на основе правил или определяемые данными выражения символики.

This feature was developed by [Nyall Dawson](https://api.github.com/users/nyalldawson)
### Добавлена ​​базовая поддержка времени для векторных слоев
Это открывает возможность применения для векторных слоев некоторых базовых возможностей временны́х данных:
- Статический временной диапазон для слоя (чтобы соответствовать возможностям растрового слоя), это устанавливает один статический временной диапазон, который применяется ко всему слою. Все элементы слоя будут отображаться всякий раз, когда время холста накладывается с временным диапазоном слоя.
...
Некоторые известные ограничения / недоработки:
- В настоящее время могут использоваться только поля Date / DateTime. Это было сделано, чтобы упростить обработку формата и избежать необходимости беспокоиться о строковых полях с различными форматами DateTime. В будущем нам следует разрешить выбор строковых полей и разрешить пользователям вводить пользовательскую строку формата DateTime.
...
Таким образом, в настоящее время производительность на больших слоях намного хуже по сравнению с Time Manager (и предоставляемый набор функций меньше, например, нет поддержки интерполяции), но преимущество в том, что мы можем использовать собственную временную структуру и анимировать векторные слои вместе с mesh сетками и растровыми слоями!

This feature was developed by [Nyall Dawson](https://api.github.com/users/nyalldawson)
### Поддержка API временны́х растров Postgres
Добавлены возможности интеграции параметра времени для растров Postgres.
...
Спонсорская поддержка - ARPA Piemonte

This feature was developed by [Alessandro Pasotti](https://api.github.com/users/elpaso)
### Настройки времени в проекте QGIS
В свойства теперь проекта добавлены настройки времени.
...
Эта базовая настройка для реализации функций анимации временны́х слоев в QGIS.
...
This feature was developed by [Kartoza / Samweli Mwakisambwe](https://kartoza.com)
### Поддержка временны́х ограничений для WMS-T слоев.
Эта функция добавляет возможность ограничения Временны́х слоев WMS (WMS-T) внутри QGIS. Вы сможете ограничить временной диапазон для любого временно́го слоя из источника WMS-T.

Это позволяет пользователю указать требуемый временной диапазон для слоя на вкладке источника данных в диалоговом окне свойств слоя.

Below is an example of the workflow. ![image6](images/entries/74638436-ee0f5c00-517c-11ea-938e-cec23c61ba29.gif)

**Примечание:** *Для запросов определённой точки во времени указание временно́го диапазона с одинаковым началом и концом приведет к использованию указанного момента времени, а не диапазона.*
...
This feature was developed by [Kartoza / Samweli Mwakisambwe](https://kartoza.com)
### API временны́х данных
Этот функционал обеспечивает реализацию API для поддержки временны́х данных в QGIS.
...
This feature was developed by [Kartoza / Samweli Mwakisambwe](https://kartoza.com)
## Инструменты
###  Поддержка Векторных тайлов инструментом Информации об объектах.
Инструмент Информации об объектах теперь умеет показывать атрибуты для объектов на Слое векторных тайлов.
...
### Рядом с кнопками виджета масштаба добавлено меню упрощающее настройку виджета в соответствии с масштабом карты на макете печати.
Наглядная демонстрация вместо тысячи слов:
...
### Перетаскивание слоёв между разными окнами QGIS
...
### Открытие Таблиц Атрибутов в виде вкладок
...
## Стиль
### Рендеринг Контуров для Растровых слоёв
Этот новый рендерер рисует контурные линии, которые рассчитываются на лету из исходного растра. Можно установить интервал контурных линий и символ, используемый для рисования.
...
### Процент добавлен в качестве единицы размера Растрового маркера и Заливки растром.
...
Например, слой содержит растровые изображения различных размеров и соотношений сторон:
...
Если мы используем Растровый маркер с **единицами размера по умолчанию**, слой будет выглядеть следующим образом (с обычным крокодилом, но огромным жирафом):

![image17](images/entries/77846711-bfec5580-71d9-11ea-9afe-a52af9b2784e.webp)

или вот так (с обычным жирафом, но крошечным крокодилом):

![image18](images/entries/77846764-1eb1cf00-71da-11ea-9d6d-f468708ea726.webp)

Новая **процентная** единица измерения предназначена для решения этой проблемы.
...
### Возможность назначать Семейство шрифтов / Стиль для Символьного маркера на основе формульных данных.
...
This feature was developed by [Mathieu Pellerin](https://api.github.com/users/nirvn)
### Новое свойство Стиль шрифта для Символьного маркера
Благодаря этой новой настройке Cтиля шрифта пользователи могут выбирать нестандартные семейства шрифтов, ранее недоступные.
...
This feature was developed by [Mathieu Pellerin](https://api.github.com/users/nirvn)
## Подписывание
### Цвета в нотации HTML в Подписях
Если включен, этот параметр будет обрабатывать содержимое подписи как HTML, и любые свойства форматирования HTML, относящиеся к ЦВЕТУ ОБЪЕКТОВ ПЕРЕДНЕГО ПЛАНА, будут учитываться в отображаемой подписи.
...
С другой стороны, это корректно работает со ВСЕМИ другими настройками подписей, включая тени, буферы, изогнутые метки и т. д.!

И несколько красивых картинок:
...
This feature was developed by [Nyall Dawson](https://api.github.com/users/nyalldawson)
### Автоматическое размещение подписей за границами полигонов
Эта функция добавляет возможность автоматического размещения подписей вне полигональных объектов.
...
This feature was developed by [Nyall Dawson](https://api.github.com/users/nyalldawson)
### Векторные тайловые слои - часть 4 (подписывание)
Продолжается работа по поддержке векторных тайловых слоёв.
...
This feature was developed by [Lutra Consulting (Martin Dobias)](https://www.lutraconsulting.co.uk/)
### Дополнительный контроль над узловой точкой для выноски подписи
Это дает пользователям управлять тем, где выноска должна присоединяться к тексту подписи (ранее у вас был только контроль над тем, где выноска будет присоединяться к соответствующей геометрии объекта).

Выбор включает в себя следующие варианты:
- Ближайшая точка (ранее реализованное поведение)
- Центроид Подписи
- Фиксированные углы: Вверху слева/Вверху справа/Внизу слева/Внизу справа/и т. д.

Также возможно управление привязкой подписи на основе формульных данных.

Помогает исправить некоторые неряшливые выноски!

Результат работы старого алгоритма (режим ближайшей точки):
...
### Скалярные настройки цвета в зависимости от классификации
С помощью этой функции, MDAL считывает классифицированные скалярные значения в наборе данных. Когда значения классифицируются, в настройках скалярного рендеринга по умолчанию используются градиент и метки, установленные в соответствии с классифицированными значениями.
...
This feature was developed by [Lutra Consulting (Vincent Cloarec)](https://www.lutraconsulting.co.uk/)
### Привязка к элементам mesh
В API добавлены методы для реализации привязки к элементам mesh сетки.

Метод возвращает позицию привязанной точки на ближайшем элементе mesh сетки:
- Для вершины позиция привязки - это позиция вершины
- Для ребра позиция  привязки - это проецируемая точка на ребре, или край ребра, если за пределами ребра
- Для грани позиция привязки - это  центроид грани

Позиция возвращается в координатах карты.
...
This feature was developed by [Nyall Dawson](https://api.github.com/users/nyalldawson)
### Возможность вставки изображений непосредственно в макеты
Встраивание вставленной картинки в макет.
...
Добавляет два новых стиля масштабной линейки (которые доступны в ArcMap, но ранее было невозможно воспроизвести в QGIS)
...
This feature was developed by [Nyall Dawson](https://api.github.com/users/nyalldawson)
### Управление Системой Координат через переменную
Это даёт возможность управлять Системой Координат Карты посредством переменной
...
Разработчики - Etienne Trimaille, Jan Caha, Julien Monticolo, Nyall Dawson
...
Спонсорская поддержка - Кантон Золотурн и федеральная земля Форарльберг
...
Спонсорская поддержка - ecoptima и Planteam
...
1. Создать случайный растровый слой (биномиальное распределение)
2. Создать случайный растровый слой (экспоненциальное распределение)
3. Создать случайный растровый слой (гамма распределение)
4. Создать случайный растровый слой (отрицательное биномиальное распределение)
5. Создать случайный растровый слой (нормальное распределение)
6. Создать случайный растровый слой (распределение Пуассона)
7. Создать случайный растровый слой (равномерное распределение)
...
Спонсорская поддержка - Andreas Neumann
...
Спонсорская поддержка - Fisel + König
...
This feature was developed by [Clemens Raffler](https://api.github.com/users/root676)
### Различные исправления в Инструментах Анализа
- Finalize work on range parameters support in GRASS algorithms;
- Исправлен неработающий алгоритм r.series
- Небольшие исправления в алгоритмах r.rescale и r.rescale.eq
- Исправление Инструмента Растеризации
...
### Feature: Webp
> WebP - это современный формат изображений, который обеспечивает превосходное сжатие без потерь и с потерями для изображений в Интернете. WebP создает меньшие по размеру файла изображения, которые делают Интернет быстрее.
> 
> Размеры изображений без потерь в WebP на 26% меньше по сравнению с PNG. Изображения с потерями в WebP на 25-34% меньше, чем сопоставимые изображения JPEG с эквивалентным индексом качества SSIM.
...
### Исправления ошибок от Alessandro Pasotti
| Название ошибки | URL адрес на issues.qgis.org (если было сообщено) | URL адрес Коммита (Github) | Коммит обратного порта для 3.10  (GitHub) |
| --- | --- | --- | --- |
| Getfeatureinfo не показывает элементы отношений для QGIS Server | [#36549](https://github.com/qgis/QGIS/issues/36549) | Переведено в разряд предложений | н/д |
| провайдер OGR QgsOgrProvider::changeAttributeValues возвращает true при ошибках | [#36583](https://github.com/qgis/QGIS/issues/36583) | [PR #36620](https://github.com/qgis/QGIS/pull/36620) | н/д |
| Группы транзакций GPKG хранятся с пустой второй парой ключей | [#36525](https://github.com/qgis/QGIS/issues/36525) | [PR #36660](https://github.com/qgis/QGIS/pull/36660) | [PR #37157](https://github.com/qgis/QGIS/pull/37157) |
| Символика, использующая отношения, не работает в QGIS Server | [#36614](https://github.com/qgis/QGIS/issues/36614) | не воспроизводимо | н/д |
| \"Loading PostGIS raster with QGIS 3.12.3 crashes\" | [#36689](https://github.com/qgis/QGIS/issues/36689) | [PR #36702](https://github.com/qgis/QGIS/pull/36702) | не применимо |
| Значение GetFeatureInfo Fid отличается при запросе через application/json и application/vnd.ogc.gml | [#36262](https://github.com/qgis/QGIS/issues/36262) | [PR #36733](https://github.com/qgis/QGIS/pull/36733) | [PR #37158](https://github.com/qgis/QGIS/pull/37158) |
| Уникальные ограничения уровня базы данных для GPKG (и spatialite) не поддерживаются в OGR (и spatialite) | [#36468](https://github.com/qgis/QGIS/issues/36468) | [PR #36802](https://github.com/qgis/QGIS/pull/36802) | н/д |
| нет значения в форме атрибута, если значение отсутствует в карте значений | [#32756](https://github.com/qgis/QGIS/issues/32756) | [PR #36804](https://github.com/qgis/QGIS/pull/36804) | н/д |
| Менеджер баз данных больше не показывает результаты запроса | [#36205](https://github.com/qgis/QGIS/issues/36205) | [PR #36823](https://github.com/qgis/QGIS/pull/36823) | н/д |
| Менеджер баз данных больше не показывает результаты запроса | [#36205](https://github.com/qgis/QGIS/issues/36205) | [PR #36831](https://github.com/qgis/QGIS/pull/36831) | н/д |
| Перетаскивание в браузере  не работает для GPKG | не зарегистрировано | [PR #36972](https://github.com/qgis/QGIS/pull/36972) | [PR #37159](https://github.com/qgis/QGIS/pull/37159) |
| невозможно сохранить проект в geopackage | [#36832](https://github.com/qgis/QGIS/issues/36832) | [PR #36980](https://github.com/qgis/QGIS/pull/36980) | н/д |
| It is possible to store text into a numeric field, while it shouldn\'t be, using the Date/Time edit widget | [#36715](https://github.com/qgis/QGIS/issues/36715) | [#36715](https://github.com/qgis/QGIS/issues/36715) | н/д |
| Создание объектов очень медленно при использовании связей | [#36167](https://github.com/qgis/QGIS/issues/36167) | [PR #36866](https://github.com/qgis/QGIS/pull/36866), [PR #36963](https://github.com/qgis/QGIS/pull/36963) | н/д |
| Сервер QGIS: WFS Request не использует SrsName для геометрии | [#36398](https://github.com/qgis/QGIS/issues/36398) | [PR #36926](https://github.com/qgis/QGIS/pull/36926), [PR #37071](https://github.com/qgis/QGIS/pull/37071) | не критично |
| Невозможно удалить сохраненное соединение для geopackage из браузера | [#36930](https://github.com/qgis/QGIS/issues/36930) | [PR #36937](https://github.com/qgis/QGIS/pull/36937) | [PR #37166](https://github.com/qgis/QGIS/pull/37166) |
| Ограничения: уникальное ограничение не корректно оценивается, когда 0 | [#36962](https://github.com/qgis/QGIS/issues/36962) | [PR #36967](https://github.com/qgis/QGIS/pull/36967) | н/д |
| PyQgis: поля OriginJoin не представлены в контейнере QgsLayerVector QgsFields для слоев GeoPackage | [#36977](https://github.com/qgis/QGIS/issues/36977) | не воспроизводимо | н/д |
| Joined tables cannot be edited even if the \"editable join layer\" option is checked | [#36934](https://github.com/qgis/QGIS/issues/36934) | не воспроизводимо | н/д |
| Removal of unused \'allowMethod\' from qgisservice/qgisserverapi | [#36163](https://github.com/qgis/QGIS/issues/36163) | [PR #36997](https://github.com/qgis/QGIS/pull/36997) | н/д |
| Гомогенизация заголовка окна | [#35761](https://github.com/qgis/QGIS/issues/35761) | [PR #36998](https://github.com/qgis/QGIS/pull/36998) | н/д |
| Подписывание: невозможно активировать опцию формульных данных для настройки выравнивания | [#37003](https://github.com/qgis/QGIS/issues/37003) | [PR #37006](https://github.com/qgis/QGIS/pull/37006) | [PR #37167](https://github.com/qgis/QGIS/pull/37167) |
| QGIS reports \"no layers\" in GeoPackage when geopackage directory is write protected | [#36574](https://github.com/qgis/QGIS/issues/36574) | [PR #37018](https://github.com/qgis/QGIS/pull/37018) | не критично (только интерфейс пользователя) |
| Joined tables cannot be edited even if the \"editable join layer\" option is checked | [#36934](https://github.com/qgis/QGIS/issues/36934) | won\'t fix | н/д |
| Отметки времени Postgis из запроса в Менеджере Баз данных отображаются как \ `PyQT5.QTCore.QDateTime (2020, 3, 17 | [#35140](https://github.com/qgis/QGIS/issues/35140) | [PR #37042](https://github.com/qgis/QGIS/pull/37042) | н/д |
| Неправильный предпросмотр изображения и размер предварительного просмотра изображения в форме | [#33682](https://github.com/qgis/QGIS/issues/33682) | [PR #37093](https://github.com/qgis/QGIS/pull/37093) | [PR #37168](https://github.com/qgis/QGIS/pull/37168) |
...
| Попытка создать новый объект в Spatialite с группой транзакций и ограничением NOT NULL завершается неудачей | [#37236](https://github.com/qgis/QGIS/issues/37236) | won\'t fix | н/д |
| Using \"Add feature\" to create feature and child feature with transaction group causes foreign key error | [#37222](https://github.com/qgis/QGIS/issues/37222) | [PR #37249](https://github.com/qgis/QGIS/pull/37249) | не критично |
| В векторном слое spatialite отсутствуют объекты и неправильно считается их количество | [#29264](https://github.com/qgis/QGIS/issues/29264) | [PR #37277](https://github.com/qgis/QGIS/pull/37277) | TODO |

This feature was funded by [QGIS.ORG (through donations and sustaining memberships)](https://www.qgis.org/)

This feature was developed by [Alessandro Pasotti](https://www.itopen.it/)
### Исправления ошибок от Loïc Bartoletti
| Название ошибки | URL адрес на issues.qgis.org (если было сообщено) | URL адрес Коммита (Github) | Коммит обратного порта для 3.10  (GitHub) |
| --- | --- | --- | --- |
| Исправлена ​​ошибка, когда у векторного слоя есть Z координата | [#32016](https://github.com/qgis/QGIS/issues/32016) | [PR #36553](https://github.com/qgis/QGIS/pull/36553) | [PR #36595](https://github.com/qgis/QGIS/pull/36595) |
| Исправлено построение прямоугольника по 3-м точкам | [#35043](https://github.com/qgis/QGIS/issues/35043), [#35671](https://github.com/qgis/QGIS/issues/35671) | [PR #36523](https://github.com/qgis/QGIS/pull/36523) | [PR #36560](https://github.com/qgis/QGIS/pull/36560) |
| Разрешён поиск qmlplugindump по разным путям | н/д | [PR #36513](https://github.com/qgis/QGIS/pull/36513) | н/д |
| Исправлено отсутствие слов в расширенном виджете конфигурации | [PR #35643](https://github.com/qgis/QGIS/pull/35643)#issuecomment-610215386 | [PR #36511](https://github.com/qgis/QGIS/pull/36511) | н/д |
| Исправления интерполяции для Разбитой геометрии | [PR #36514](https://github.com/qgis/QGIS/pull/36514) | [#33489](https://github.com/qgis/QGIS/issues/33489) | н/д |

This feature was funded by [QGIS.ORG (through donations and sustaining memberships)](https://www.qgis.org/)

This feature was developed by [Loïc Bartoletti](https://www.oslandia.com/)
### Исправления ошибок от Even Rouault
| Название ошибки | URL адрес на issues.qgis.org (если было сообщено) | URL адрес Коммита (Github) | Коммит обратного порта для 3.10  (GitHub) |
| --- | --- | --- | --- |
| Неправильная интерпретация кода EPSG из файла PRJ в QGIS-dev (proj v7.1.0) | [#36111](https://github.com/qgis/QGIS/issues/36111) | [PROJ PR 2240](https://github.com/OSGeo/PROJ/pull/2240) | не применимо: проблема в PROJ |
...
| Плохое сообщение об ошибке при сбое подключения WFS | [#29866](https://github.com/qgis/QGIS/issues/29866) | [PR #36717](https://github.com/qgis/QGIS/pull/36717) | [PR #36735](https://github.com/qgis/QGIS/pull/36735) |
| Сбой QGIS во время повторного рендеринга карты после переключения Системы координат | [#29672](https://github.com/qgis/QGIS/issues/29672) | не воспроизводимо | н/д |
...
| Проблема перепроецирования растров в Системах координат 54019 и 54042 | [#35512](https://github.com/qgis/QGIS/issues/35512) | [PROJ PR 2243](https://github.com/OSGeo/PROJ/pull/2243) | не применимо: проблема в PROJ |
| ПРЕДУПРЕЖДЕНИЕ от провайдера WFS - Невозможно создать временный кэш SpatiaLite при установке через flatpak. | [#36545](https://github.com/qgis/QGIS/issues/36545) | [PR #36745](https://github.com/qgis/QGIS/pull/36745) | н/д |
| Транзакция WFS с использованием URI GetCapabilities | [#34307](https://github.com/qgis/QGIS/issues/34307) | [PR #36746](https://github.com/qgis/QGIS/pull/36746) | н/д |
| QGis 3.10.2 аварийно завершает работу при попытке отображения двух растровых слоев из PostGIS | [#34456](https://github.com/qgis/QGIS/issues/34456) | - | н/д |
| Сбой QGIS 3.4.10 - Python | [#31304](https://github.com/qgis/QGIS/issues/31304) | Уже исправлено в последней версии | [PR #36751](https://github.com/qgis/QGIS/pull/36751) |
| Провайдер WMS ожидает, ключевое слово SERVICE=WMTS в верхнем регистре для обнаружения WMTS | [#36659](https://github.com/qgis/QGIS/issues/36659) | [PR #36752](https://github.com/qgis/QGIS/pull/36752) | [PR #36766](https://github.com/qgis/QGIS/pull/36766) |
| Ответ WCS DescribeCoverage всегда пуст | [#36504](https://github.com/qgis/QGIS/issues/36504) | [PR #36754](https://github.com/qgis/QGIS/pull/36754) | [PR #36781](https://github.com/qgis/QGIS/pull/36781) |
| Неправильный экстент, если слой содержит точку в 0,0 | [#33823](https://github.com/qgis/QGIS/issues/33823) | Ошибка не связана с QGIS. Исправлено в GDAL 3.1 | н/д |
| Проекция не распознается из конвертации из Mapinfo | [#34471](https://github.com/qgis/QGIS/issues/34471) | [PR #36758](https://github.com/qgis/QGIS/pull/36758) | [PR #36783](https://github.com/qgis/QGIS/pull/36783) |
| QGIS падает/зависает при добавлении большого CSV-файла в виде текстового слоя с разделителями | [#36392](https://github.com/qgis/QGIS/issues/36392) | [PR #36778](https://github.com/qgis/QGIS/pull/36778) | [PR #36810](https://github.com/qgis/QGIS/pull/36810) |
| QGIS 3.12 неизвестная Система координат или выбор Преобразования | [#36837](https://github.com/qgis/QGIS/issues/36837) | [GDAL Commit 68133b8](https://github.com/OSGeo/gdal/commit/68133b8b6f724a581a5dcb19fbdcd657ddbd6c4c) | [GDAL Commit 6ae7f60](https://github.com/OSGeo/gdal/commit/6ae7f60a5914a2f16b8b1a94758f10e6621e4aca) |
| Ошибка BIGTIFF не очевидна в журнале обработки | [#36867](https://github.com/qgis/QGIS/issues/36867) | [GDAL Commit cfc8e61](https://github.com/OSGeo/gdal/commit/cfc8e613ad86dd4b00604dd64df239916ccda135) | [GDAL Commit fb88e3c](https://github.com/OSGeo/gdal/commit/fb88e3ca5c8905fbcaab79e96a2bfc1c36f72d08) |

This feature was funded by [QGIS.ORG (through donations and sustaining memberships)](https://www.qgis.org/)

This feature was developed by [Even Rouault](http://www.spatialys.com/)
### Исправление ошибок - Paul Blottiere
| Название ошибки | URL адрес на issues.qgis.org (если было сообщено) | URL адрес Коммита (Github) | Коммит обратного порта для 3.10  (GitHub) |
| --- | --- | --- | --- |
| Исправлена ​​ошибка при добавлении слоя | [Mailing list](https://lists.osgeo.org/pipermail/qgis-developer/2020-June/061421.html) | [PR #36910](https://github.com/qgis/QGIS/pull/36910) | н/д |
| Исправлен сигнал подключения | О проблеме не сообщалось | [PR #35836](https://github.com/qgis/QGIS/pull/35836) | н/д |
| Обновление API для экспорта легенды в JSON | Проблема привязки о которой не сообщалось | [PR #36370](https://github.com/qgis/QGIS/pull/36370) | н/д |
| Дамп ядра, если Слой векторных тайлов (файл) становится недоступным | [#36821](https://github.com/qgis/QGIS/issues/36821) | не воспроизводимо | н/д |
| Сбой при открытии проекта, когда mapcanvas extents = nan | [#35899](https://github.com/qgis/QGIS/issues/35899) | не воспроизводимо | н/д |
| Использование некоторых функций в растровом калькуляторе ведёт к сбою в QGIS | [#35583](https://github.com/qgis/QGIS/issues/35583) | [PR #37273](https://github.com/qgis/QGIS/pull/37273) | н/д |
| Вспомогательное хранилище невозможно использовать в нередактируемом слое | [#30376](https://github.com/qgis/QGIS/issues/30376) | Уже исправлено. | н/д |
| Исправления для корректной компиляции в GCC 10, Qt/PyQt 5.15.0 и SIP 4.19.23 | [#37072](https://github.com/qgis/QGIS/issues/37072) | [PR #37116](https://github.com/qgis/QGIS/pull/37116) | [PR #37250](https://github.com/qgis/QGIS/pull/37250) |

This feature was funded by [QGIS.ORG (through donations and sustaining memberships)](https://www.qgis.org/)

This feature was developed by [Paul Blottiere](https://hytech-imaging.fr/)
### Исправления ошибок от Julien Cabieces
| Название ошибки | URL адрес на issues.qgis.org (если было сообщено) | URL адрес Коммита (Github) | Коммит обратного порта для 3.10  (GitHub) |
| --- | --- | --- | --- |
| Воспроизводимый сбой с использованием QgsMapCanvas.items () со слоями Oracle | [#33791](https://github.com/qgis/QGIS/issues/33791) | [PR #36240](https://github.com/qgis/QGIS/pull/36240) | [PR #36246](https://github.com/qgis/QGIS/pull/36246) |
| Разрывы строк игнорируются в панели справки пользовательских выражений | [#36191](https://github.com/qgis/QGIS/issues/36191) | [PR #36249](https://github.com/qgis/QGIS/pull/36249) | [PR #36387](https://github.com/qgis/QGIS/pull/36387) |
| \"Edit user expression\" does not allow to rename the user expression | [#36192](https://github.com/qgis/QGIS/issues/36192) | [PR #36349](https://github.com/qgis/QGIS/pull/36349) | н/д |
| Невозможно изменить подписи классов растровых символов | [#36172](https://github.com/qgis/QGIS/issues/36172) | [PR #36376](https://github.com/qgis/QGIS/pull/36376) | [PR #36533](https://github.com/qgis/QGIS/pull/36533) |
| Сбой, когда слой недоступен | [#33300](https://github.com/qgis/QGIS/issues/33300) | не воспроизводимо | н/д |
| Выборка по выражению возвращает ошибочные результаты с целочисленным делением поля для шейп-файлов | [#35449](https://github.com/qgis/QGIS/issues/35449) | [PR #2531](https://github.com/OSGeo/gdal/pull/2531) | н/д |
...
| Зависание в окне последних проектов при открытии пустого файла в 3.10.3 | [#34809](https://github.com/qgis/QGIS/issues/34809) | не воспроизводимо | н/д |
| Сбой QGIS при получении элементов сцены mapCanvas, если объекты определены | [#34457](https://github.com/qgis/QGIS/issues/34457) | [PR #36439](https://github.com/qgis/QGIS/pull/36439) | [PR #36450](https://github.com/qgis/QGIS/pull/36450) |
| сортировка по столбцу, с настроенным отношением, является непоследовательной | [#36114](https://github.com/qgis/QGIS/issues/36114) | [PR #36776](https://github.com/qgis/QGIS/pull/36776) | [PR #36887](https://github.com/qgis/QGIS/pull/36887) |
| Потеря точек (в зависимости от масштаба холста) в виртуальном слое на основе файла sqlite | [#36054](https://github.com/qgis/QGIS/issues/36054) | [PR #36792](https://github.com/qgis/QGIS/pull/36792) | [PR #36718](https://github.com/qgis/QGIS/pull/36718) |
| Проект считается изменённым, как только вы перемещаете курсор | [#36796](https://github.com/qgis/QGIS/issues/36796) | [PR #36797](https://github.com/qgis/QGIS/pull/36797) | [PR #36718](https://github.com/qgis/QGIS/pull/36718) |
...
### Исправления ошибок от Bertrand Rix
| Название ошибки | URL адрес на issues.qgis.org (если было сообщено) | URL адрес Коммита (Github) | Коммит обратного порта для 3.10  (GitHub) |
| --- | --- | --- | --- |
| Инструмент редактирования вершин переопределяет глобальные параметры привязки | [#36229](https://github.com/qgis/QGIS/issues/36229) | [PR #36231](https://github.com/qgis/QGIS/pull/36231) | н/д |
| Сбой QGIS при попытке использования метода QgsGeometry на пустой коллекции | [#36142](https://github.com/qgis/QGIS/issues/36142) | [PR #36351](https://github.com/qgis/QGIS/pull/36351) | н/д |
| Выборочное маскирование: фантомные линии на темном фоне | [#34650](https://github.com/qgis/QGIS/issues/34650) | [PR #36697](https://github.com/qgis/QGIS/pull/36697) | н/д |
| Выборочное маскирование: проблема с непрозрачностью маски | [#34947](https://github.com/qgis/QGIS/issues/34947) | [PR #36697](https://github.com/qgis/QGIS/pull/36697) | н/д |

This feature was funded by [QGIS.ORG (through donations and sustaining memberships)](https://www.qgis.org/)

This feature was developed by [Bertrand Rix](https://www.oslandia.com/)
### Исправления ошибок от Sebastien Peillet
| Название ошибки | URL адрес на issues.qgis.org (если было сообщено) | URL адрес Коммита (Github) | Коммит обратного порта для 3.10  (GitHub) |
| --- | --- | --- | --- |
| \[Oracle\]Error on save geometries because wrong default values #34482 | [#34482](https://github.com/qgis/QGIS/issues/34482) | [PR #36769](https://github.com/qgis/QGIS/pull/36769) | н/д |
| Неверный запрос для значения по умолчанию столбца даты слоя Oracle | [#32401](https://github.com/qgis/QGIS/issues/32401) | [PR #36769](https://github.com/qgis/QGIS/pull/36769) | н/д |
| Поля, отсутствуют в списке выбора для "Видимости по выражению" для группы в форме перетаскивания | [#35196](https://github.com/qgis/QGIS/issues/35196) | [PR #36824](https://github.com/qgis/QGIS/pull/36824) | н/д |
| Пользовательские масштабы проекта отформатированы в научной нотации | [#36859](https://github.com/qgis/QGIS/issues/36859) | [PR #36860](https://github.com/qgis/QGIS/pull/36860) | н/д |
| Oracle: таблицы перечислены один раз для полигонов и один раз для мультиполигонов, но обе показывают всю геометрию | [#32521](https://github.com/qgis/QGIS/issues/32521) | [PR #34358](https://github.com/qgis/QGIS/pull/34358) | н/д |
| Can\'t set min/max values to decimal in raster symbology with QGis Linux versions | [#33859](https://github.com/qgis/QGIS/issues/33859) | [PR #37136](https://github.com/qgis/QGIS/pull/37136) | н/д |

This feature was funded by [QGIS.ORG (through donations and sustaining memberships)](https://www.qgis.org/)

This feature was developed by [Sebastien Peillet](https://www.oslandia.com/)
### Исправления ошибок от Alexander Bruy
| Название ошибки | URL адрес на issues.qgis.org (если было сообщено) | URL адрес Коммита (Github) | Коммит обратного порта для 3.10  (GitHub) |
| --- | --- | --- | --- |
| При копировании усеченной переменной копируется усеченный текст, а не оригинал | [#30641](https://github.com/qgis/QGIS/issues/30641) | [PR #36554](https://github.com/qgis/QGIS/pull/36554) | [PR #36576](https://github.com/qgis/QGIS/pull/36576) |
| Отключение режима Мультиправки не возвращает к режиму просмотра таблицы | [#25099](https://github.com/qgis/QGIS/issues/25099) | [PR #36541](https://github.com/qgis/QGIS/pull/36541) | [PR #36630](https://github.com/qgis/QGIS/pull/36630) |
| Инструменты GRASS: отсутствует название параметра | [#36520](https://github.com/qgis/QGIS/issues/36520) | [PR #36644](https://github.com/qgis/QGIS/pull/36644) | [PR #36690](https://github.com/qgis/QGIS/pull/36690) |
| Исправлено перенаправление результатов стандартного вывода во временные файлы в алгоритмах обработки GRASS. | не зарегистрировано | [PR #36618](https://github.com/qgis/QGIS/pull/36618) | [PR #36684](https://github.com/qgis/QGIS/pull/36684) |
| Исправлен алгоритм GRASS r.tileset | не зарегистрировано | [PR #36645](https://github.com/qgis/QGIS/pull/36645) | [PR #36695](https://github.com/qgis/QGIS/pull/36695) |
| QgsRasterTerrainAnalysisPlugin: проверка значения возвращаемого QgsRelief::processRaster() | [#17452](https://github.com/qgis/QGIS/issues/17452) | [PR #36731](https://github.com/qgis/QGIS/pull/36731) | [PR #36740](https://github.com/qgis/QGIS/pull/36740) |
| Гистограмма для градуированного стиля в свойствах слоя исчезает, когда окно свойств слоя слишком маленькое | [#25197](https://github.com/qgis/QGIS/issues/25197) | [PR #36732](https://github.com/qgis/QGIS/pull/36732) | [PR #36749](https://github.com/qgis/QGIS/pull/36749) |
...
| Экспорт стиля растра в SLD работает только тогда, когда выходной файл имеет суффикс .sld | [#35944](https://github.com/qgis/QGIS/issues/35944) | [PR #36774](https://github.com/qgis/QGIS/pull/36774) | [PR #36795](https://github.com/qgis/QGIS/pull/36795) |
| \[Style Manager\] Missing Favorites category in the \"select by group\" dialog from \"Export symbols\" | [#27315](https://github.com/qgis/QGIS/issues/27315) | [PR #36793](https://github.com/qgis/QGIS/pull/36793) | не критично |
| TIN Интерполяция - проблемы с форматом и размером выходного растра | [#31970](https://github.com/qgis/QGIS/issues/31970) | [PR #36822](https://github.com/qgis/QGIS/pull/36822) | [PR #36844](https://github.com/qgis/QGIS/pull/36844) |
| Layer panel: \"Edit Symbol\" does nothing if no symbol defined for a rule (in rule-based rendering) | [#23048](https://github.com/qgis/QGIS/issues/23048) | [PR #36954](https://github.com/qgis/QGIS/pull/36954) | не критично |
| Смещение атрибутов в результатах алгоритма "Область обслуживания" | не зарегистрировано | [PR #37074](https://github.com/qgis/QGIS/pull/37074), [PR #37097](https://github.com/qgis/QGIS/pull/37097) | н/д |
| Среда тестирования не применяет округление к значениям, приведенным к числам | не зарегистрировано | [PR #37234](https://github.com/qgis/QGIS/pull/37234) | н/д |

This feature was funded by [QGIS.ORG (through donations and sustaining memberships)](https://www.qgis.org/)

Разработчик - Alexander Bruy
### Исправления ошибок от Nyall Dawson
| Название ошибки | URL адрес на issues.qgis.org (если было сообщено) | URL адрес Коммита (Github) | Коммит обратного порта для 3.10  (GitHub) |
| --- | --- | --- | --- |
| Исправить утечку в рендерере 3d mesh сетки | не зарегистрировано | [Commit 4b5a63d](https://github.com/qgis/QGIS/commit/4b5a63d11b7ea5baf6931c8abf86e132c706c1be) | н/д |
| Исправлено зависание при рендеринге символов с использованием размера в Метрах в Масштабе карты | [#28690](https://github.com/qgis/QGIS/issues/28690) | [Commit 7c6286f](https://github.com/qgis/QGIS/commit/7c6286f8dfb7cc24e5b06c9ab950d9dd55fac517) | No \-- too intrusive |
| Исправлен шрифт в редакторе кода CSS | не зарегистрировано | [Commit 63cb42c](https://github.com/qgis/QGIS/commit/63cb42c298ef5ab732274bce205e49171b797ae4) | не критично |
| Исправлен рендеринг выражений генератора геометрии, которые возвращают Коллекции геометрии в качестве результата | [#35356](https://github.com/qgis/QGIS/issues/35356) | [Commit be281c49](https://github.com/qgis/QGIS/commit/be281c49b7932a1d0d0e3cb4dce7af17f5b201a9) | No \-- too intrusive |
| \[processing\] Correctly expose date time inputs (and a bunch of others) as inputs for in-model expressions | [#37219](https://github.com/qgis/QGIS/issues/37219) | [Commit 281356a](https://github.com/qgis/QGIS/commit/281356aa4e951db5cd2782b53a7261c7343c4ca7) | н/д |
| Исправьте все предупреждения об устаревании Qt 5.14 | не зарегистрировано | [Commit 3717adc](https://github.com/qgis/QGIS/commit/3717adcef6315f4b9a0089bfc2393cede937b644) | No \-- too intrusive |
...
| Fix for container \"show as group box\" setting is lost when closing layer properties dialog | [#37205](https://github.com/qgis/QGIS/issues/37205) | [Commit 803f507](https://github.com/qgis/QGIS/commit/803f507d45f99534dbc03ab5300e6137da36cd59) | н/д |
| Более настойчиво отговаривать пользователей от создания пользовательских проекций с использованием формата Proj строки | [#37204](https://github.com/qgis/QGIS/issues/37204) | [Commit 7ec4968](https://github.com/qgis/QGIS/commit/7ec4968aa9dcb2107b00c1caaeb6ae575498bc3c) | No \-- string change |
...
| Исправлена ​​обработка нумерованных списков в dox/PyQGIS API | не зарегистрировано | [Commit 74400c7](https://github.com/qgis/QGIS/commit/74400c7ff88d563dad74dad41860f9b603214973) | Пропускать |
| Исправление перекрестных ссылок в документах PyQGIS | не зарегистрировано | [Commit 81a9db9](https://github.com/qgis/QGIS/commit/81a9db9f57706cba9b11e4372dd1bf50fcd7bc6a) | Пропускать |
| \[layouts\] Fix attribute table sort order combined with feature filter results in empty tables | [#36341](https://github.com/qgis/QGIS/issues/36341) | [Commit 24897ab](https://github.com/qgis/QGIS/commit/24897ab86a7f11be196d8d705fa3307b05e0e53d) | н/д |
| Исправлен рендеринг выноски для повернутых подписей | [#36681](https://github.com/qgis/QGIS/issues/36681) | [Commit 2bb4aad](https://github.com/qgis/QGIS/commit/2bb4aad8c9b7238bd6ff6dc9a90a28c8625e859e) | н/д |
| Исправлена ​​обработка вывода grass в Моделях | [#36379](https://github.com/qgis/QGIS/issues/36379) | [Commit ef19911](https://github.com/qgis/QGIS/commit/ef19911f1132dc5720a4068f059a0322d7511e2b) |  |
| Исправление неправильного упрощения векторных данных до прямоугольников в некоторых Системах координат | [#36898](https://github.com/qgis/QGIS/issues/36898) | [Commit 47fba10](https://github.com/qgis/QGIS/commit/47fba106dd9d057e6691dcca392efacbe31ad9ff) | No \-- too intrusive |
| Исправлено форматирование элементов списка в документации PyQGIS | не зарегистрировано | [Commit 84ec481](https://github.com/qgis/QGIS/commit/84ec481aa8f3d3bb487272122414c765e77a2ea2) | н/д |
| \[processing\] Fix manual entry of extent values in widget | [#36787](https://github.com/qgis/QGIS/issues/36787) | [Commit ccc34c7](https://github.com/qgis/QGIS/commit/ccc34c76e714e5f6f87d2a329ca048896eb4c87f) | н/д |
...
| Исправлен сбор типов геометрии треугольников после Тесселяции | [#36638](https://github.com/qgis/QGIS/issues/36638) | [Commit df46fba](https://github.com/qgis/QGIS/commit/df46fba33823415b8310c986795f8c51550a0fa6) | No \-- too intrusive |
| Исправлена ошибка при записи типа геометрии треугольника через QgsVectorFileWriter | [#36638](https://github.com/qgis/QGIS/issues/36638) | [Commit e0c03f9](https://github.com/qgis/QGIS/commit/e0c03f9f0848490f358876ae5be6973961700bc9) | No \-- too intrusive |
| Исправлено ухудшение результатов Тесселяции | [#37077](https://github.com/qgis/QGIS/issues/37077) | [Commit 61e7a5f](https://github.com/qgis/QGIS/commit/61e7a5fc90154bd4d35fea7d6cbd413ce61fa02e) | н/д |
| Исправлено игнорирование строки подмножества слоя, когда OGR отказывается принимать скомпилированное выражение запроса объекта | [#37073](https://github.com/qgis/QGIS/issues/37073) | [Commit b5b4221](https://github.com/qgis/QGIS/commit/b5b42218af5cbfca82f3b0a4d5a521e3d330f550) | [PR 36718 Commit f486cf6](https://github.com/qgis/QGIS/pull/36718/commits/f486cf6e6dadb2d0690f167620c90f4efddb3c4a) |
...
| Убраны некоторых предупреждений при запуске QGIS | не зарегистрировано | [Commit 013f682](https://github.com/qgis/QGIS/commit/013f682bd3c6addd163a78d3776c578ae37377c6) | н/д |
| Перемещение вкладки 3d в свойствах векторного слоя на правильное место | [#35010](https://github.com/qgis/QGIS/issues/35010) | [Commit ce9f5194](https://github.com/qgis/QGIS/commit/ce9f519468d46bb6fe6dd0bc68efd7d58129be68) | н/д |
| Исправлено: 3d вкладка не активируется при повторном открытии свойств векторного слоя | [#35011](https://github.com/qgis/QGIS/issues/35011) | [Commit 3c0ecbf](https://github.com/qgis/QGIS/commit/3c0ecbfd79e1924cd01143324cdc26fe4a68f748) | н/д |
| \[labeling\] Fix incorrect label placement after moving an \'unplaced\' label | не зарегистрировано | [Commit cbfb7f4](https://github.com/qgis/QGIS/commit/cbfb7f4ce25e10aab5f7b8cee147afb801042704) | [PR 36718 Commit 9189dec](https://github.com/qgis/QGIS/pull/36718/commits/9189dec7633b2eb6a1075bb35c2f90f6b90ba63a) |
| Don\'t block creation of new geopackages in vector save as dialog | не зарегистрировано | [Commit 8ccd127](https://github.com/qgis/QGIS/commit/8ccd127f4d8bf66fb04b6484147ed116acf66d91) | н/д |
...
| \[3d\] When showing edges, data defined polygon height was not accounted for | не зарегистрировано | [Commit e516231](https://github.com/qgis/QGIS/commit/e516231bb2854405ea78a0cec40ad96295e9f8c6) | н/д |
| Fixes for \"Show all\" labeling mode | не зарегистрировано | [Commit 2b91bd0](https://github.com/qgis/QGIS/commit/2b91bd0cafff66f14ceb49b104bf2f2cc6ae714d) | н/д |
| \[3d\] Fix reprojection of features shows no features | [#34824](https://github.com/qgis/QGIS/issues/34824) | [Commit 62bd026](https://github.com/qgis/QGIS/commit/62bd0267439a65f94a5cd9cb886bbb4e4780cdf0) | н/д |
| \[processing\] Add metadata setting for file destination parameters to avoid file overwrite confirmation prompt | [#37011](https://github.com/qgis/QGIS/issues/37011) | [Commit e81af2c](https://github.com/qgis/QGIS/commit/e81af2c9e10dc2a2e220c815cb9e531f7dba838d) | н/д |
| Исправление некорректного сглаживания при рендеринге определенных объектов (например, подписей), когда они не нужны | [#36964](https://github.com/qgis/QGIS/issues/36964) | [Commit bab83844](https://github.com/qgis/QGIS/commit/bab838444cb55fc479d2f3dfa3b19d8ce80e73af) | No \-- too intrusive |
| Проверка, что поля, требуемые для фоновой заливки задника подписей, выбираются при рендеринге | [#36944](https://github.com/qgis/QGIS/issues/36944) | [Commit 8fb8e45](https://github.com/qgis/QGIS/commit/8fb8e45d516611f09bf28e3832c3c6214f16c4a2) | No \-- too intrusive |
| \"Fix \"\"zoom\"\" and \"\"flash\"\" features buttons have no effect when opening filter mode in attribute form\" | [#34506](https://github.com/qgis/QGIS/issues/34506) | [Commit 6b2795](https://github.com/qgis/QGIS/commit/6b27958948db2c88ae72b0b9ff99fc8aa8522bf2) | н/д |
| Избежание риска поврежденных эскизов проекта в некоторых обстоятельствах | не зарегистрировано | [Commit f404e6e](https://github.com/qgis/QGIS/commit/f404e6e162df954ffb200d550b6c32375b7270c1) | н/д |
| Auto focus search box when opening \"add layer to legend\" dialog | [#36905](https://github.com/qgis/QGIS/issues/36905) | [Commit 019e494](https://github.com/qgis/QGIS/commit/019e494f1d61851ea192900a2cc9cce460d1ea73) | не критично |
| \[processing\] Set correct output layer type for Point to Layer algorithm | [#36941](https://github.com/qgis/QGIS/issues/36941) | [Commit 7ee85cd](https://github.com/qgis/QGIS/commit/7ee85cd93cb5e0b623feaad2ef1ce9ea8f654bac) | н/д |
...
| Запомнинание расположение окон Дизайнера макетов | [#36907](https://github.com/qgis/QGIS/issues/36907) | [Commit 97d31d0](https://github.com/qgis/QGIS/commit/97d31d025fe2264afadcf6afcb3eb6ebcfe9e91d) | не критично |
| Исправлено аварийное завершение при слишком большом приближении к подписям | [#36346](https://github.com/qgis/QGIS/issues/36346) | [Commit 3aa5e56](https://github.com/qgis/QGIS/commit/3aa5e56d3c3dbb61f2a28c4874593aa401957ed3) | [Commit a22635b](https://github.com/qgis/QGIS/commit/a22635bb64ae6d2a46715bd1208463e22b6080ae) |
...
| Fix placement of message bar \'clear all\' menu arrow on hidpi screens | не зарегистрировано | [Commit 8a9975b](https://github.com/qgis/QGIS/commit/8a9975b4ff6b7a8a79bef05230db28754500fa40) | не критично |
| Обновление репозитория плагинов, при ручном запуске пользователем  | [#34351](https://github.com/qgis/QGIS/issues/34351) | [Commit 98603b7](https://github.com/qgis/QGIS/commit/98603b78660e05334c7b34ea3722533164f9553b) | [PR 36718 Commit 0fcf6dc](https://github.com/qgis/QGIS/pull/36718/commits/0fcf6dc08289fb549c9bace24a70ed4a50bdd959) |
...
| \[layouts\] Don\'t refresh the map canvas multiple times during atlas exports | [#30144](https://github.com/qgis/QGIS/issues/30144) | [Commit 57519e9](https://github.com/qgis/QGIS/commit/57519e9faa95fbfe6196134d9128caee48cb4b84) | не критично |
...
| Исправлено неверное предупреждающее сообщение при отключении предварительного просмотра атласа | не зарегистрировано | [Commit 3b0af960](https://github.com/qgis/QGIS/commit/3b0af960dd90af15cbe32e2fd7642408031e70f5) | н/д |
| Ускорение работы панели сообщений, когда их отображается много | [#29698](https://github.com/qgis/QGIS/issues/29698) | [Commit 5dd29f0](https://github.com/qgis/QGIS/commit/5dd29f065e57fde532a21ae1bc9311aef9378455) | No \-- too intrusive |
| Исправлен расчет минимальной ограничительной рамки, который в некоторых случаях давал неверный результат | [#36632](https://github.com/qgis/QGIS/issues/36632) | [Commit 97d0988](https://github.com/qgis/QGIS/commit/97d098884a7889346739d61bd9048e9dcf942d11) | [PR 36718 Commit 68c6a0e](https://github.com/qgis/QGIS/pull/36718/commits/68c6a0e31dc58fa23621e872739df9efded6e6b7) |
| Проверка что непрозрачность символа всегда игнорируется при выделении | [#27859](https://github.com/qgis/QGIS/issues/27859) | [Commit 4de6a53](https://github.com/qgis/QGIS/commit/4de6a53235a6f7bd657c60d4f4d7160ed371d9df) | [PR 36718 Commit 99eb710](https://github.com/qgis/QGIS/pull/36718/commits/99eb710f89be0c6adc9acfdd888e366e6f2b796f) |
| Исправлен случайный порядок сортировки символов в диалоге Экспорта Символов | [#20572](https://github.com/qgis/QGIS/issues/20572) | [Commit 987310f](https://github.com/qgis/QGIS/commit/987310fe90de2b21deb5c2564eb051e1d72548b2) | [PR 36718 Commit bfa6058](https://github.com/qgis/QGIS/pull/36718/commits/bfa60586e10735a6b670cd349dd8b9efd2ab2762) |
| Разрешено обнулять поле весов тепловой карты | [#20256](https://github.com/qgis/QGIS/issues/20256) | [Commit 3961fa2](https://github.com/qgis/QGIS/commit/3961fa26f53e411c5c83b50e006df739ebda4c3e) | [PR 36718 Commit d2600d6](https://github.com/qgis/QGIS/pull/36718/commits/d2600d6d2cb765cb0641347ac342a74f663e4ba5) |
| Flag SAGA \'Merge Layers\' algorithm as having known issues | [#36375](https://github.com/qgis/QGIS/issues/36375) | [Commit f676c821](https://github.com/qgis/QGIS/commit/f676c821d9d9ff2d6e509d881ba2ea1853a34051) | н/д |
| Обход проблемы в библиотеке Qt , когда для полупрозрачной заливки без сплошного узора неправильно применяется прозрачность к контуру при отправке на принтер | [#36580](https://github.com/qgis/QGIS/issues/36580) | [Commit 38c8218](https://github.com/qgis/QGIS/commit/38c82187a65d8b4833f2c633a69a4d937a8725eb) | [PR 36718 Commit a562b67](https://github.com/qgis/QGIS/pull/36718/commits/a562b678438d26c1f80486fdfff422d70c8c6329) |
| Улучшения в контроллере плагинов и его интерфейсе  | не зарегистрировано | [Commit 150b373](https://github.com/qgis/QGIS/commit/150b373c7ae61c0548ab0803f9b30a36b15e7ab4) | No \-- too intrusive |
...
| Avoid proj \'object is not a concatenated operation\' warnings | не зарегистрировано | [Commit 6baa209](https://github.com/qgis/QGIS/commit/6baa209285f452e5996537b1b0f1b5d924d5c59b) |  |
...
| Реализовано обнаружение пространственного индекса для провайдера OGR | [#30530](https://github.com/qgis/QGIS/issues/30530) | [Commit f20801a](https://github.com/qgis/QGIS/commit/f20801a8bb3f23576e5e0d2abc6d3229e5044200) | [PR 36718 Commit 536906e](https://github.com/qgis/QGIS/pull/36718/commits/536906ec6375b70d88767e58a08550369a25d1a0) |
| Исправлен неверный заголовок диалога Свойств растрового слоя | [#35510](https://github.com/qgis/QGIS/issues/35510) | [Commit 2099261](https://github.com/qgis/QGIS/commit/2099261230f82c5cd4439538e38754947c8f6bd6) | [PR 36718 Commit e809f48](https://github.com/qgis/QGIS/pull/36718/commits/e809f48cb9aaf03dfe8671c2d885f99b4a91922b) |
| Don\'t try to identify CRSes created just for ellipsoid definitions | не зарегистрировано | [Commit 15b318b](https://github.com/qgis/QGIS/commit/15b318b50de9131ed1dd6bbf73ae5eb7572579a7) | No \-- too intrusive |

This feature was funded by [QGIS.ORG (through donations and sustaining memberships)](https://www.qgis.org/)

This feature was developed by [Nyall Dawson](https://north-road.com/)
### Исправления ошибок от Denis Rouzaud
| Название ошибки | URL адрес на issues.qgis.org (если было сообщено) | URL адрес Коммита (Github) | Коммит обратного порта для 3.10  (GitHub) |
| --- | --- | --- | --- |
| Разрешена сортировка таблицы атрибутов по полю, не отображаемому в таблице | [#25671](https://github.com/qgis/QGIS/issues/25671) | [PR #36236](https://github.com/qgis/QGIS/pull/36236) | н/д |
| исправлены зависимости дерева слоев (символы showinf и основной слой) | не зарегистрировано | [PR #37000](https://github.com/qgis/QGIS/pull/37000) | н/д |
| исправлен редактор выражений: проверка синтаксиса завершается неудачно, если таблица пуста | [#37100](https://github.com/qgis/QGIS/issues/37100) | [PR #37137](https://github.com/qgis/QGIS/pull/37137) | н/д |
| исправлена ​​настройка функции в виджете предварительного просмотра выражений | [#37214](https://github.com/qgis/QGIS/issues/37214) | [PR #37221](https://github.com/qgis/QGIS/pull/37221) | н/д |
| Исправлено зависание при отображении таблицы атрибутов слоя WFS | [#37224](https://github.com/qgis/QGIS/issues/37224) | ещё не исправлено | н/д |
| Поиски в таблицах с более чем 100 записями показывал неправильные значения в Связанных отношением частях формы | [#37266](https://github.com/qgis/QGIS/issues/37266) | [PR #37280](https://github.com/qgis/QGIS/pull/37280) | [PR #37286](https://github.com/qgis/QGIS/pull/37286) |

This feature was funded by [QGIS.ORG (through donations and sustaining memberships)](https://www.qgis.org/)

This feature was developed by [Denis Rouzaud](https://www.opengis.ch/)
### Исправления ошибок от Audun Ellertsen
| Название ошибки | URL адрес на issues.qgis.org (если было сообщено) | URL адрес Коммита (Github) | Коммит обратного порта для 3.10  (GitHub) |
| --- | --- | --- | --- |
| Поддержка бинарными полями при импорте SpatiaLite слоёв | [#36705](https://github.com/qgis/QGIS/issues/36705) | [PR #36744](https://github.com/qgis/QGIS/pull/36744) | н/д |
