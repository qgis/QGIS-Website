...
with open("data/conf.json", "w") as f:
    json.dump({
        ...
        "binary": lr_binary,
        "codename": lr_name.replace("'", "\\'"),
        "releasenote": lr_note.replace("'", "\\'") if lr_note != '' else ('RC' if lr_version.split('.')[2] == '0' else '\\u200B'),
        "ltrversion": ltrversion,
        "ltrrelease": ltr_version,
        "ltrcodename": ltr_name.replace("'", "\\'"),
        "ltrbinary": ltr_binary,
        "ltrnote": ltr_note.replace("'", "\\'") if ltr_note != '' else 'LTR',
        "shortver": shortver,
