    ...
    def save_contributors(self, contributors: Dict, output_path: str):
        ...
        result = {
            "_automated_warning": {
                ...
                "warning": "This file is automatically generated. Do NOT edit manually! (You can change your location coordinates in the file data/contributors/contributors_location.json.)",
...
def generate_geojson(contributors_json_path: str, output_geojson_path: str):
    """
    Generate GeoJSON file from contributors.json by matching with QGIS contributors geometry data.
    
    Args:
        contributors_json_path: Path to data/contributors/contributors.json
        output_geojson_path: Path to output GeoJSON file
    """
    print("\nGenerating GeoJSON from contributors data...")
    
    # Load our contributors data
    if not os.path.exists(contributors_json_path):
        print(f"Error: Contributors file not found: {contributors_json_path}")
        return False
    ...
    for contributor in sorted_contributors:
        ...
        # Skip email-based or name-based contributors
        if login.startswith("email:") or login.startswith("name:") or login.startswith("no-github:"):
            continue
        
        # Try to find geometry by matching login with GIT Nickname
        geometry = git_nickname_to_geometry.get(login)
        
        # Flatten thematics into individual properties
        thematics = contributor.get("thematics", {})
        properties = {
            "login": login,
            "avatar_url": contributor.get("avatar_url", ""),
            "total_contributions": contributor.get("total_contributions", 0),
            "has_github_account": contributor.get("has_github_account", True)
        }
        ...
        if geometry:
            contributors_with_geometry += 1
        else:
            contributors_without_geometry += 1
    ...
    geojson_output = {
        "_automated_warning": {
            "generated_by": "scripts/update_individual_contributors.py",
            "warning": "This file is automatically generated. Do NOT edit manually! (You can change your location coordinates in the file data/contributors/contributors_location.json.)",
            "how_to_update": "Run 'python scripts/update_individual_contributors.py --geojson-only' or with --token to update both files",
            "documentation": "See CONTRIBUTING.md section: Automated Content and Manual Edit Guidelines"
        },
        "generated_at": datetime.now(timezone.utc).isoformat(),
    ...
    print(f"  Total contributors: {len(features)}")
    print(f"  With geometry: {contributors_with_geometry}")
    print(f"  Without geometry: {contributors_without_geometry}")
...
def main():
    ...
    parser.add_argument('--geojson-only', action='store_true', 
                       help='Only generate GeoJSON from existing contributors.json without updating contributor data')
    ...
    geojson_output_path = "static/data/contributors/contributors_map.json"
    
    # If only generating GeoJSON
    if args.geojson_only:
        print("Running in GeoJSON-only mode (not updating contributors.json)")
        success = generate_geojson(output_path, geojson_output_path)
    ...
    if not token:
        ...
        print("Or use --geojson-only to only generate GeoJSON from existing data.")
        return 1
    ...
    try:
        ...
        if contributors_data:
            ...
            # Now generate GeoJSON from the updated contributors.json
            print("\nGenerating GeoJSON file...")
            generate_geojson(output_path, geojson_output_path)
