---
type: "page"
title: "Downloads"
subtitle: "Get OSGeo software"
description: "Download free and open source geospatial software from OSGeo. Desktop GIS, web mapping, spatial databases, and more."
draft: false
heroSize: "is-medium"
HasBanner: true
ButtonText: "Try OSGeoLive"
ButtonLink: "/initiatives/osgeolive/"
LabelText: "All Free & Open Source"
sidebar: true
---

{{< columns-start >}}
{{< column-start class="is-8" >}}

<div class="section-header">
<div class="section-header-badge">
<i class="fa-solid fa-download"></i>
<span>Software</span>
</div>
<h2 class="section-title">Download OSGeo Software</h2>
</div>

All OSGeo software is free and open source. Download, use, modify, and share freely.

---

<div class="section-header">
<div class="section-header-badge">
<i class="fa-solid fa-compact-disc"></i>
<span>Quick Start</span>
</div>
<h2 class="section-title">Quick Start: OSGeoLive</h2>
</div>

The easiest way to try OSGeo software is with OSGeoLive - a self-contained bootable DVD/USB with 50+ pre-configured geospatial applications.

{{< rich-box-start icon="fa-solid fa-compact-disc" >}}
### OSGeoLive

Try 50+ open source geospatial applications without installing anything. Perfect for:

- Learning and evaluation
- Training and workshops
- Demonstrations
- Air-gapped environments

**Includes:** QGIS, GRASS GIS, PostGIS, GeoServer, MapServer, OpenLayers, and many more.

[Download OSGeoLive](/initiatives/osgeolive/){.button .is-osgeo-cta}
{{< rich-box-end >}}

---

<div class="section-header">
<div class="section-header-badge">
<i class="fa-solid fa-desktop"></i>
<span>Desktop</span>
</div>
<h2 class="section-title">Desktop Applications</h2>
</div>

Full-featured geographic information systems for your desktop.

{{< columns-start >}}

{{< column-start class="is-6" >}}
{{< rich-box-start icon="fa-solid fa-desktop" >}}
### QGIS Desktop

The leading free and open source desktop GIS.

**Platforms:** Windows, macOS, Linux

[Download QGIS](https://qgis.org/download/)
{{< rich-box-end >}}
{{< column-end >}}

{{< column-start class="is-6" >}}
{{< rich-box-start icon="fa-solid fa-desktop" >}}
### GRASS GIS

Powerful raster, vector, and geospatial processing.

**Platforms:** Windows, macOS, Linux

[Download GRASS GIS](https://grass.osgeo.org/download/)
{{< rich-box-end >}}
{{< column-end >}}

{{< columns-end >}}

{{< columns-start >}}

{{< column-start class="is-6" >}}
{{< rich-box-start icon="fa-solid fa-desktop" >}}
### gvSIG Desktop

User-friendly GIS with CAD-like editing.

**Platforms:** Windows, macOS, Linux

[Download gvSIG](http://www.gvsig.com/en/products/gvsig-desktop/downloads)
{{< rich-box-end >}}
{{< column-end >}}

{{< column-start class="is-6" >}}
{{< rich-box-start icon="fa-solid fa-globe" >}}
### Marble

Virtual globe and world atlas.

**Platforms:** Windows, macOS, Linux

[Download Marble](https://marble.kde.org/install.php)
{{< rich-box-end >}}
{{< column-end >}}

{{< columns-end >}}

---

<div class="section-header">
<div class="section-header-badge">
<i class="fa-solid fa-server"></i>
<span>Web</span>
</div>
<h2 class="section-title">Web Mapping Servers</h2>
</div>

Publish geospatial data on the web.

{{< columns-start >}}

{{< column-start class="is-6" >}}
{{< rich-box-start icon="fa-solid fa-server" >}}
### GeoServer

Share, process, and edit geospatial data with open standards.

[Download GeoServer](https://geoserver.org/release/stable/)
{{< rich-box-end >}}
{{< column-end >}}

{{< column-start class="is-6" >}}
{{< rich-box-start icon="fa-solid fa-server" >}}
### MapServer

High-performance web mapping platform.

[Download MapServer](https://mapserver.org/download.html)
{{< rich-box-end >}}
{{< column-end >}}

{{< columns-end >}}

---

<div class="section-header">
<div class="section-header-badge">
<i class="fa-solid fa-database"></i>
<span>Databases</span>
</div>
<h2 class="section-title">Spatial Databases</h2>
</div>

Store and query spatial data at scale.

{{< columns-start >}}

{{< column-start class="is-6" >}}
{{< rich-box-start icon="fa-solid fa-database" >}}
### PostGIS

Spatial extension for PostgreSQL.

[Download PostGIS](https://postgis.net/install/)
{{< rich-box-end >}}
{{< column-end >}}

{{< columns-end >}}

---

<div class="section-header">
<div class="section-header-badge">
<i class="fa-solid fa-code"></i>
<span>Libraries</span>
</div>
<h2 class="section-title">Libraries & Tools</h2>
</div>

Core libraries that power geospatial applications.

{{< columns-start >}}

{{< column-start class="is-4" >}}
{{< rich-box-start icon="fa-solid fa-code" >}}
### GDAL/OGR

Data translation library.

[Download GDAL](https://gdal.org/download.html)
{{< rich-box-end >}}
{{< column-end >}}

{{< column-start class="is-4" >}}
{{< rich-box-start icon="fa-solid fa-code" >}}
### PROJ

Coordinate transformations.

[Download PROJ](https://proj.org/download.html)
{{< rich-box-end >}}
{{< column-end >}}

{{< column-start class="is-4" >}}
{{< rich-box-start icon="fa-solid fa-code" >}}
### GEOS

Geometry engine.

[Download GEOS](https://libgeos.org/usage/download/)
{{< rich-box-end >}}
{{< column-end >}}

{{< columns-end >}}

---

<div class="section-header">
<div class="section-header-badge">
<i class="fa-brands fa-js"></i>
<span>JavaScript</span>
</div>
<h2 class="section-title">JavaScript Libraries</h2>
</div>

Build interactive web maps.

{{< columns-start >}}

{{< column-start class="is-6" >}}
{{< rich-box-start icon="fa-brands fa-js" >}}
### OpenLayers

High-performance mapping library.

[Get OpenLayers](https://openlayers.org/download/)
{{< rich-box-end >}}
{{< column-end >}}

{{< column-start class="is-6" >}}
{{< rich-box-start icon="fa-brands fa-js" >}}
### Leaflet

Lightweight interactive maps.

[Get Leaflet](https://leafletjs.com/download.html)
{{< rich-box-end >}}
{{< column-end >}}

{{< columns-end >}}

---

<div class="section-header">
<div class="section-header-badge">
<i class="fa-solid fa-cubes"></i>
<span>All</span>
</div>
<h2 class="section-title">All Projects</h2>
</div>

View the complete catalog of OSGeo projects:

[Browse All Projects](/projects/){.button .is-osgeo-primary .is-medium}

{{< column-end >}}
{{< column-start class="is-4" >}}

{{< box-start classes="has-background-osgeo-light" >}}
### Start Here

**New to OSGeo software?**

Download OSGeoLive to try everything at once.

[OSGeoLive](/initiatives/osgeolive/)
{{< box-end >}}

{{< box-start classes="has-background-light mt-4" >}}
### Package Managers

OSGeo software is available through:

**Linux:**
- Ubuntu/Debian apt
- Fedora dnf
- conda-forge

**macOS:**
- Homebrew
- MacPorts

**Cross-platform:**
- conda
- Docker images
{{< box-end >}}

{{< box-start classes="has-background-white mt-4" >}}
### Get Support

- [Mailing Lists](/community/mailing-lists/)
- [IRC Chat](/community/irc/)
- [Service Providers](/service-providers/)
{{< box-end >}}

{{< box-start classes="has-background-light mt-4" >}}
### Source Code

All projects are open source:

[OSGeo GitHub](https://github.com/osgeo)

[GitLab](https://gitlab.com/osgeo)
{{< box-end >}}

{{< column-end >}}
{{< columns-end >}}
